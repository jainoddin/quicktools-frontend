...
export function promptMetadata(title: string, description: string, canonical: string): Metadata {
  // Root layout adds " | QuickTools.ai" (16 chars). Keep the page-specific
  // portion compact so dynamic prompt titles do not become oversized in SERPs.
  ...
  const expandedDescription = cleanDescription.length < 110 ? `${cleanDescription.replace(/[.!?]?$/, '.')}${helpfulSuffix}` : cleanDescription;
  ...
  return {
    ...
    openGraph: { type: 'website', url: canonical, title: metaTitle, description: metaDescription, siteName: 'QuickTools.ai', images: [{ url: PROMPT_OG_IMAGE, width: 1200, height: 630, alt: 'QuickTools.ai Prompt Hub' }] },
