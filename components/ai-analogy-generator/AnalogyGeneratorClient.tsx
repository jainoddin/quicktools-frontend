...
import { Download, History, Loader2, Copy, CheckCircle2, Sparkles, RefreshCw, Share2, Building2, Megaphone, Lightbulb } from 'lucide-react';
...
export default function AnalogyGeneratorClient() {
  ...
  const localStorageKey = 'guest_ai-analogy-generator_history';
  const toolSlug = '/tools/ai-analogy-generator';
          ...
          if (data.success && data.data?.history) {
            const items = data.data.history
              ...
              .map((item: any) => ({
                ...
                prompt: item.prompt,
                contentType: 'AI Analogy Generator',
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch(getEndpoint('/api/tools/ai-analogy-generator'), {
        method: 'POST',
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-8 h-full">
      ...
      <aside className="w-full lg:w-[340px] shrink-0 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both">
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            <label className="flex items-center gap-2 text-sm font-bold text-[#111827]">
              Enter the complex topic or jargon
            </label>
          ...
          <textarea
            ...
            className="w-full h-40 p-4 bg-white rounded-2xl border border-[#E5E7EB] text-sm text-[#4B5563] placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#06B6D4]/20 focus:border-[#06B6D4] transition-all resize-none shadow-sm"
          />
        </div>
        ...
        <div className="pt-2">
          <button
            ...
            style={{ backgroundColor: isProcessing || !inputVal.trim() ? '' : '#06B6D4' }}
          >
            {isProcessing ? (
              <><Loader2 className="w-5 h-5 animate-spin" /> Generating...</>
            ) : (
              <><Lightbulb className="w-5 h-5" /> Generate Results</>
            )}
          </button>
        </div>
      ...
      {showHistory ? (
        ...
        <main className="flex-grow flex flex-col min-w-0">
          ...
          <div className="flex flex-col md:flex-row md:items-start lg:items-center justify-between gap-4 mb-6">
            <div>
              <h1 className="text-3xl lg:text-4xl font-bold tracking-tight text-[#111827] flex items-center gap-2">AI Analogy Generator <Sparkles className="w-6 h-6" style={{ color: '#06B6D4' }} /></h1>
              <p className="text-[#6B7280] text-sm lg:text-base mt-2">
                Explain complex tech or legal jargon using simple analogies from sports, movies, or hobbies.
              </p>
            </div>
                ...
                <div className="flex flex-wrap items-center gap-3 pt-6 border-t border-[#E5E7EB] shrink-0">
                  ...
                  <button
                    onClick={async () => {
                      if (navigator.share) {
                        try {
                          await navigator.share({ title: 'AI Analogy Generator', text: result });
                        } catch (err) {
      ...
      <TextDownloadModal 
        ...
        content={result} 
        filename="AI Analogy Generator" 
        toolSlug="ai-analogy-generator" 
