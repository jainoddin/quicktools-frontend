...
import { Download, SmilePlus, History, Loader2, Copy, CheckCircle2, Sparkles, Target, RefreshCw, Share2, MessageSquare, Hash } from 'lucide-react';
...
export default function AiEmojiTranslatorClient() {
  ...
  const localStorageKey = 'guestAiEmojiTranslatorHistory';
  const toolSlug = '/tools/ai-emoji-translator';
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch(getEndpoint('/api/tools/emoji-translator'), {
        method: 'POST',
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-8 h-full">
      ...
      <aside className="w-full lg:w-[340px] shrink-0 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both">
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="e.g., I am going to the beach tomorrow to surf and drink coconut water"
            className="w-full h-40 p-4 bg-white rounded-2xl border border-[#E5E7EB] text-sm text-[#4B5563] placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#FBBF24]/20 focus:border-[#FBBF24] transition-all resize-none shadow-sm"
          />
        </div>
        ...
        <div className="pt-2">
          <button
            ...
            className={`w-full text-white font-bold py-3.5 rounded-xl transition-all shadow-md hover:shadow-lg flex items-center justify-center gap-2 ${isProcessing || !inputVal.trim() ? 'bg-[#9CA3AF] cursor-not-allowed' : 'bg-[#FBBF24] hover:opacity-90'}`}
          >
            {isProcessing ? (
              <><Loader2 className="w-5 h-5 animate-spin" /> Generating...</>
            ) : (
              <><SmilePlus className="w-5 h-5" /> Translate to Emojis</>
            )}
          </button>
        </div>
        ...
        <div className="mt-4 pt-4 border-t border-[#E5E7EB]">
          ...
          <div className="flex flex-col gap-2">
            <a href="/tools/ai-caption-generator" className="flex items-center gap-3 p-2.5 bg-[#F9FAFB] hover:bg-[#EC4899]/5 rounded-xl transition-colors group">
              ...
              <div>
                <p className="text-[11px] font-bold text-[#111827] group-hover:text-[#EC4899] transition-colors">AI Caption Generator</p>
                <p className="text-[9px] text-[#6B7280]">Generate viral social captions</p>
              </div>
            </a>
            <a href="/tools/ai-hashtag-generator" className="flex items-center gap-3 p-2.5 bg-[#F9FAFB] hover:bg-[#06B6D4]/5 rounded-xl transition-colors group">
              <div className="w-8 h-8 shrink-0 rounded-lg bg-white border border-[#E5E7EB] flex items-center justify-center group-hover:border-[#06B6D4]/30 group-hover:shadow-sm transition-all">
                <Hash className="w-4 h-4" style={{color:'#06B6D4'}} />
              </div>
              <div>
                <p className="text-[11px] font-bold text-[#111827] group-hover:text-[#06B6D4] transition-colors">AI Hashtag Generator</p>
                <p className="text-[9px] text-[#6B7280]">Find trending hashtags</p>
              </div>
            </a>
          </div>
        </div>
      ...
      {showHistory ? (
        ...
        <main className="flex-grow flex flex-col min-w-0">
          ...
          <div className="flex flex-col md:flex-row md:items-start lg:items-center justify-between gap-4 mb-6">
            <div>
              <h1 className="text-3xl lg:text-4xl font-bold tracking-tight text-[#111827] flex items-center gap-2">AI Emoji Translator <Sparkles className="w-6 h-6" style={{ color: '#FBBF24' }} /></h1>
              <p className="text-[#6B7280] text-sm lg:text-base mt-2">
                Translates plain English text into creative emoji sequences.
              </p>
            </div>
          ...
          <div className="flex-grow bg-white border border-[#E5E7EB] rounded-3xl p-6 lg:p-8 shadow-sm flex flex-col h-[600px] animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both delay-150">
            {isProcessing ? (
              <TextGenerationProgress title="Translating..." description="Converting your text to expressive emojis." />
            ) : result ? (
              <>
                ...
                <div className="flex flex-wrap items-center gap-3 pt-6 border-t border-[#E5E7EB] shrink-0">
                  ...
                  <button
                    onClick={async () => {
                      if (navigator.share) {
                        try {
                          await navigator.share({ title: 'A I  Emoji Translator', text: result });
                        } catch (err) {
      ...
      <TextDownloadModal 
        ...
        content={result} 
        filename="A I  Emoji Translator" 
        toolSlug="ai-emoji-translator" 
