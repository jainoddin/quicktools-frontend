...
import { 
  Video, History, Loader2, Copy, CheckCircle2,
...
export default function AiYoutubeTitleClient() {
  ...
  useEffect(() => {
    if (!isAuthenticated) {
      const saved = localStorage.getItem('guestVideoTitleHistory');
        ...
        .then(data => {
          if (data.success && data.data?.history) {
            const items = data.data.history
              .filter((item: any) => item.toolSlug === '/tools/ai-youtube-title')
              .map((item: any) => ({
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch(getEndpoint('/api/tools/youtube-title'), {
        method: 'POST',
      ...
      if (data.success) {
        ...
        if (!isAuthenticated) {
          const historyItem = {
            ...
            prompt: videoTopic.substring(0, 100),
            contentType: 'YouTube Titles',
          ...
          localStorage.setItem('guestVideoTitleHistory', JSON.stringify(newHistory));
        } else {
          setToolHistory(prev => [{
            id: data.usageId || Date.now().toString(),
            prompt: videoTopic.substring(0, 100),
      ...
      <aside className="w-full lg:w-[340px] shrink-0 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both">
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-xs text-[#6B7280]">{videoTopic.length}/500</span>
          </div>
          <textarea
            value={videoTopic}
            onChange={(e) => setVideoTopic(e.target.value.substring(0, 500))}
            placeholder="e.g., How to build a custom mechanical keyboard from scratch for beginners..."
        ...
        <div className="pt-2">
          <button
            ...
            disabled={isProcessing || !videoTopic.trim()}
            className={`w-full text-white font-bold py-3.5 rounded-xl transition-all shadow-md hover:shadow-lg flex items-center justify-center gap-2 ${isProcessing || !videoTopic.trim() ? 'bg-[#9CA3AF] cursor-not-allowed' : 'bg-[#EF4444] hover:bg-[#DC2626]'}`}
          >
            {isProcessing ? (
              <><Loader2 className="w-5 h-5 animate-spin" /> Brainstorming...</>
            ) : (
              <><Video className="w-5 h-5" /> Generate Titles</>
            )}
          </button>
        </div>
        ...
        <div className="mt-4 pt-4 border-t border-[#E5E7EB]">
          ...
          <div className="flex flex-col gap-2">
            <a href="/tools/ai-caption-generator" className="flex items-center gap-3 p-2.5 bg-[#F9FAFB] hover:bg-[#FEF2F2] rounded-xl transition-colors group">
              ...
              <div>
                <p className="text-[11px] font-bold text-[#111827] group-hover:text-[#EF4444] transition-colors">Caption Generator</p>
                <p className="text-[9px] text-[#6B7280]">Captions for your shorts</p>
              </div>
            </a>
          </div>
        </div>
        ...
        <main className="flex-grow flex flex-col min-w-0">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-2xl font-bold text-[#111827] flex items-center gap-2">
                    AI YouTube Title Generator <Sparkles className="w-5 h-5 text-[#EF4444]" />
                  </h1>
                  <p className="text-sm text-[#6B7280]">Generate highly clickable, viral YouTube video titles.</p>
                </div>
              </div>
          ...
          {isProcessing ? (
            <div className="flex-grow bg-white rounded-3xl border border-[#E5E7EB] p-8 flex flex-col items-center justify-center shadow-sm animate-in fade-in duration-500 h-[600px]">
              ...
              <h2 className="text-xl font-bold text-[#111827]">Brainstorming Titles...</h2>
              <p className="text-[#6B7280] mt-2">Finding the most clickable angles for your video.</p>
            </div>
          ) : result ? (
            <div className="flex-grow flex flex-col bg-white border border-[#E5E7EB] rounded-3xl p-6 shadow-sm animate-in zoom-in-95 duration-500 h-[600px]">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-bold text-[#111827]">Video Titles</h2>
            ...
            <div className="flex-grow bg-white rounded-3xl border border-[#E5E7EB] p-8 lg:p-12 shadow-sm flex flex-col items-center justify-center text-center relative animate-in fade-in slide-in-from-bottom-8 duration-700 fill-mode-both delay-200 h-[600px]">
              ...
              <div className="relative w-72 h-64 mb-10 flex items-center justify-center mt-4">
                ...
                <div className="relative z-10 w-44 h-32 bg-white rounded-xl shadow-[0_10px_30px_-10px_rgba(239,68,68,0.3)] border-4 border-red-200 overflow-hidden flex flex-col items-center justify-center transform hover:scale-110 transition-transform duration-500">
                  <Video className="w-16 h-16 text-[#EF4444] mb-2 drop-shadow-md" />
                </div>
              ...
              <h2 className="text-2xl lg:text-3xl font-bold text-[#111827] mb-3">
                Need <span className="text-[#EF4444]">views?</span>
              </h2>
              <p className="text-[#6B7280] max-w-sm mx-auto mb-8">
                Enter your video topic on the left and click <span className="text-[#EF4444] font-semibold">"Generate Titles"</span>.
              </p>
            </div>
          )}
        </main>
