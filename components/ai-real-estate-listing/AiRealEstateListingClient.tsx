...
import { Download, Home, History, Loader2, Copy, CheckCircle2, Sparkles, Target, RefreshCw, Share2, Package, Search } from 'lucide-react';
...
export default function AiRealEstateListingClient() {
  ...
  const localStorageKey = 'guestAiRealEstateListingHistory';
  const toolSlug = '/tools/ai-real-estate-listing';
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch(getEndpoint('/api/tools/real-estate-listing'), {
        method: 'POST',
      ...
      if (data.success) {
        ...
        if (!isAuthenticated) {
          const historyItem = {
            ...
            contentType: 'Real Estate Listing Generator',
    ...
    <div className="flex flex-col lg:flex-row gap-8 h-full">
      ...
      <aside className="w-full lg:w-[340px] shrink-0 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both">
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="e.g., 3 bed, 2 bath house in Austin, newly renovated kitchen, large backyard"
        ...
        <div className="pt-2">
          <button
            ...
            className={`w-full text-white font-bold py-3.5 rounded-xl transition-all shadow-md hover:shadow-lg flex items-center justify-center gap-2 ${isProcessing || !inputVal.trim() ? 'bg-[#9CA3AF] cursor-not-allowed' : 'bg-[#10B981] hover:opacity-90'}`}
          >
            {isProcessing ? (
              <><Loader2 className="w-5 h-5 animate-spin" /> Generating...</>
            ) : (
              <><Home className="w-5 h-5" /> Generate Listing</>
            )}
          </button>
        </div>
        ...
        <div className="mt-4 pt-4 border-t border-[#E5E7EB]">
          ...
          <div className="flex flex-col gap-2">
            <a href="/tools/ai-product-description" className="flex items-center gap-3 p-2.5 bg-[#F9FAFB] hover:bg-[#6D5EF8]/5 rounded-xl transition-colors group">
              <div className="w-8 h-8 shrink-0 rounded-lg bg-white border border-[#E5E7EB] flex items-center justify-center group-hover:border-[#6D5EF8]/30 group-hover:shadow-sm transition-all">
                <Package className="w-4 h-4" style={{color:'#6D5EF8'}} />
              </div>
              <div>
                <p className="text-[11px] font-bold text-[#111827] group-hover:text-[#6D5EF8] transition-colors">AI Product Description</p>
                <p className="text-[9px] text-[#6B7280]">Write product descriptions</p>
              </div>
            </a>
            <a href="/tools/ai-seo-meta-generator" className="flex items-center gap-3 p-2.5 bg-[#F9FAFB] hover:bg-[#10B981]/5 rounded-xl transition-colors group">
              ...
              <div>
                <p className="text-[11px] font-bold text-[#111827] group-hover:text-[#10B981] transition-colors">AI SEO Meta Generator</p>
                <p className="text-[9px] text-[#6B7280]">Optimize your SEO metadata</p>
              </div>
            </a>
          </div>
        </div>
      ...
      {showHistory ? (
        <div className="flex-grow bg-white border border-[#E5E7EB] rounded-3xl p-6 shadow-sm overflow-y-auto h-[600px] animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both">
          <ToolHistorySidebar
            toolName="Real Estate Listing Generator"
            toolType="text"
            history={toolHistory}
        ...
        <main className="flex-grow flex flex-col min-w-0">
          ...
          <div className="flex flex-col md:flex-row md:items-start lg:items-center justify-between gap-4 mb-6">
            <div>
              <h1 className="text-3xl lg:text-4xl font-bold tracking-tight text-[#111827] flex items-center gap-2">AI Real Estate Listing Generator <Sparkles className="w-6 h-6" style={{ color: '#10B981' }} /></h1>
              <p className="text-[#6B7280] text-sm lg:text-base mt-2">
                Creates engaging property descriptions for real estate agents.
              </p>
            </div>
          ...
          <div className="flex-grow bg-white border border-[#E5E7EB] rounded-3xl p-6 lg:p-8 shadow-sm flex flex-col h-[600px] animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both delay-150">
            {isProcessing ? (
              <TextGenerationProgress title="Writing Listing..." description="Crafting an attractive real estate listing description." />
            ) : result ? (
              <>
                ...
                <div className="flex flex-wrap items-center gap-3 pt-6 border-t border-[#E5E7EB] shrink-0">
                  ...
                  <button
                    onClick={async () => {
                      if (navigator.share) {
                        try {
                          await navigator.share({ title: 'A I  Real Estate Listing', text: result });
                        } catch (err) {
      ...
      <TextDownloadModal 
        ...
        content={result} 
        filename="A I  Real Estate Listing" 
        toolSlug="ai-real-estate-listing" 
