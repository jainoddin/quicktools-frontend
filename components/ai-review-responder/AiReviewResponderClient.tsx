...
export default function AiReviewResponderClient() {
  ...
  const localStorageKey = 'guestAiReviewResponderHistory';
  const toolSlug = '/tools/ai-review-responder';
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch(getEndpoint('/api/tools/review-responder'), {
        method: 'POST',
      ...
      <aside className="w-full lg:w-[340px] shrink-0 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both">
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="e.g., 2 stars. The food was cold when it arrived and the delivery took 2 hours."
        ...
        <div className="pt-2">
          <button
            ...
            className={`w-full text-white font-bold py-3.5 rounded-xl transition-all shadow-md hover:shadow-lg flex items-center justify-center gap-2 ${isProcessing || !inputVal.trim() ? 'bg-[#9CA3AF] cursor-not-allowed' : 'bg-[#06B6D4] hover:opacity-90'}`}
          >
            {isProcessing ? (
              <><Loader2 className="w-5 h-5 animate-spin" /> Generating...</>
            ) : (
              <><MessageSquare className="w-5 h-5" /> Generate Reply</>
            )}
          </button>
        </div>
      </aside>
