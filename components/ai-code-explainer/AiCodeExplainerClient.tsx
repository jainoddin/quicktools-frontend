...
import { Download, Share2, RefreshCw, FileCode2, History, Loader2, Copy, CheckCircle2, Sparkles, Code } from 'lucide-react';
...
export default function AiCodeExplainerClient() {
  ...
  const handleToggleFavorite = async (id: string) => {
    setToolHistory(prev => {
      ...
      if (!isAuthenticated) {
        localStorage.setItem('guestCodeExplainerHistory', JSON.stringify(updated));
      }
      return updated;
    });
    if (isAuthenticated) {
      ...
      <aside className="w-full lg:w-[340px] shrink-0 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both">
        ...
        <div className="mt-4 pt-4 border-t border-[#E5E7EB]">
          ...
          <div className="flex flex-col gap-2">
            <a href="/tools/ai-sql-generator" className="flex items-center gap-3 p-2.5 bg-[#F9FAFB] hover:bg-[#F5F3FF] rounded-xl transition-colors group">
      ...
      {showHistory ? (
        ...
        <main className="flex-grow flex flex-col min-w-0">
          ...
          <div className="flex flex-col md:flex-row md:items-start lg:items-center justify-between gap-4 mb-6">
            <div>
              <h1 className="text-3xl lg:text-4xl font-bold tracking-tight text-[#111827] flex items-center gap-2">AI Code Explainer <Sparkles className="w-6 h-6" style={{ color: '#8B5CF6' }} /></h1>
              <p className="text-[#6B7280] text-sm lg:text-base mt-2">
                Paste any complex code and AI will explain it in simple language.
              </p>
            </div>
          ...
          <div className="flex-grow bg-white border border-[#E5E7EB] rounded-3xl p-6 lg:p-8 shadow-sm flex flex-col h-[600px] animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both delay-150">
            {isProcessing ? (
              <TextGenerationProgress title="Translating Code..." description="Breaking down logic and syntax step by step." />
            ) : result ? (
              <>
