...
import { 
  Megaphone, History, Loader2, Copy, CheckCircle2,
...
export default function AiAdCopyClient() {
  ...
  useEffect(() => {
    if (!isAuthenticated) {
      const saved = localStorage.getItem('guestAdCopyHistory');
        ...
        .then(data => {
          if (data.success && data.data?.history) {
            const items = data.data.history
              .filter((item: any) => item.toolSlug === '/tools/ai-ad-copy')
              .map((item: any) => ({
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch(getEndpoint('/api/tools/ad-copy'), {
        method: 'POST',
      ...
      if (data.success) {
        ...
        if (!isAuthenticated) {
          const historyItem = {
            ...
            prompt: productInfo.substring(0, 100),
            contentType: 'Ad Copy',
          ...
          localStorage.setItem('guestAdCopyHistory', JSON.stringify(newHistory));
        } else {
          setToolHistory(prev => [{
            id: data.usageId || Date.now().toString(),
            prompt: productInfo.substring(0, 100),
      ...
      <aside className="w-full lg:w-[340px] shrink-0 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both">
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-xs text-[#6B7280]">{productInfo.length}/500</span>
          </div>
          <textarea
            value={productInfo}
            onChange={(e) => setProductInfo(e.target.value.substring(0, 500))}
            placeholder="e.g., A new smart watch that tracks sleep and stress. 20% off this week..."
        ...
        <div className="pt-2">
          <button
            ...
            disabled={isProcessing || !productInfo.trim()}
            className={`w-full text-white font-bold py-3.5 rounded-xl transition-all shadow-md hover:shadow-lg flex items-center justify-center gap-2 ${isProcessing || !productInfo.trim() ? 'bg-[#9CA3AF] cursor-not-allowed' : 'bg-[#F97316] hover:bg-[#EA580C]'}`}
          >
            {isProcessing ? (
              <><Loader2 className="w-5 h-5 animate-spin" /> Brainstorming...</>
            ) : (
              <><Megaphone className="w-5 h-5" /> Generate Ad Copy</>
            )}
          </button>
        </div>
        ...
        <div className="mt-4 pt-4 border-t border-[#E5E7EB]">
          ...
          <div className="flex flex-col gap-2">
            <a href="/tools/ai-product-description" className="flex items-center gap-3 p-2.5 bg-[#F9FAFB] hover:bg-[#FFF7ED] rounded-xl transition-colors group">
              ...
              <div>
                <p className="text-[11px] font-bold text-[#111827] group-hover:text-[#F97316] transition-colors">Product Description</p>
                <p className="text-[9px] text-[#6B7280]">Write product copy</p>
              </div>
            </a>
          </div>
        </div>
        ...
        <main className="flex-grow flex flex-col min-w-0">
          ...
          {!result && !isProcessing && (
            <div className="flex flex-col md:flex-row md:items-start lg:items-center justify-between gap-4 mb-6 animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both delay-100">
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 bg-[#F97316] rounded-xl flex items-center justify-center shadow-sm shrink-0">
                  <Megaphone className="w-6 h-6 text-white" />
                </div>
                <div>
                  <h1 className="text-2xl font-bold text-[#111827] flex items-center gap-2">
                    AI Ad Copy Generator <Sparkles className="w-5 h-5 text-[#F97316]" />
                  </h1>
                  <p className="text-sm text-[#6B7280]">Write highly converting ad copy variations for Facebook or Google Ads.</p>
                </div>
              </div>
          ...
          {isProcessing ? (
            <div className="flex-grow bg-white rounded-3xl border border-[#E5E7EB] p-8 flex flex-col items-center justify-center shadow-sm animate-in fade-in duration-500 h-[600px]">
              ...
              <h2 className="text-xl font-bold text-[#111827]">Brainstorming Ads...</h2>
              <p className="text-[#6B7280] mt-2">Writing persuasive angles and CTAs.</p>
            </div>
          ) : result ? (
            <div className="flex-grow flex flex-col bg-white border border-[#E5E7EB] rounded-3xl p-6 shadow-sm animate-in zoom-in-95 duration-500 h-[600px]">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-bold text-[#111827]">Generated Ad Copy</h2>
                <div className="flex items-center gap-2">
                  <button 
                    onClick={copyToClipboard}
                    className="flex items-center gap-2 px-4 py-2 bg-[#FFF7ED] text-[#EA580C] rounded-xl text-sm font-bold hover:bg-[#FFEDD5] transition-colors"
                  >
              ...
              <div className="flex-1 bg-[#FAFAFA] p-8 rounded-2xl border border-[#E5E7EB] overflow-y-auto max-h-[600px] custom-scrollbar">
                <div className="prose prose-orange max-w-none text-sm text-[#374151]">
            ...
            <div className="flex-grow bg-white rounded-3xl border border-[#E5E7EB] p-8 lg:p-12 shadow-sm flex flex-col items-center justify-center text-center relative animate-in fade-in slide-in-from-bottom-8 duration-700 fill-mode-both delay-200 h-[600px]">
              ...
              <div className="relative w-72 h-64 mb-10 flex items-center justify-center mt-4">
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-56 h-56 bg-orange-50 rounded-full blur-2xl opacity-80"></div>
                
                <div className="relative z-10 w-44 h-32 bg-white rounded-xl shadow-[0_10px_30px_-10px_rgba(249,115,22,0.3)] border-4 border-orange-200 overflow-hidden flex flex-col items-center justify-center transform hover:scale-110 transition-transform duration-500">
                  <Megaphone className="w-16 h-16 text-[#F97316] mb-2 drop-shadow-md animate-[pulse_2s_infinite]" />
                </div>
              ...
              <h2 className="text-2xl lg:text-3xl font-bold text-[#111827] mb-3">
                Need <span className="text-[#F97316]">clicks?</span>
              </h2>
              <p className="text-[#6B7280] max-w-sm mx-auto mb-8">
                Enter your product/offer on the left and click <span className="text-[#F97316] font-semibold">"Generate Ad Copy"</span>.
              </p>
            </div>
          )}
        </main>
