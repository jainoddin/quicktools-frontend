...
import { 
  Lightbulb, History, Loader2, Copy, CheckCircle2,
  Sparkles, PenTool, LayoutGrid } from 'lucide-react';
...
export default function AiBlogIdeaClient() {
  ...
  useEffect(() => {
    if (!isAuthenticated) {
      const saved = localStorage.getItem('guestBlogIdeaHistory');
        ...
        .then(data => {
          if (data.success && data.data?.history) {
            const items = data.data.history
              .filter((item: any) => item.toolSlug === '/tools/ai-blog-idea-generator')
              .map((item: any) => ({
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch(getEndpoint('/api/tools/blog-idea'), {
        method: 'POST',
      ...
      if (data.success) {
        ...
        if (!isAuthenticated) {
          ...
          localStorage.setItem('guestBlogIdeaHistory', JSON.stringify(newHistory));
        } else {
          setToolHistory(prev => [{
            id: data.usageId || Date.now().toString(),
            prompt: topic.substring(0, 100),
      ...
      <aside className="w-full lg:w-[340px] shrink-0 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both">
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="e.g., Artificial Intelligence in Healthcare, Vegan Recipes for Beginners..."
        ...
        <div className="mt-4 pt-4 border-t border-[#E5E7EB]">
          ...
          <div className="flex flex-col gap-2">
            <a href="/tools/ai-writer" className="flex items-center gap-3 p-2.5 bg-[#F9FAFB] hover:bg-[#FFFBEB] rounded-xl transition-colors group">
              <div className="w-8 h-8 shrink-0 rounded-lg bg-white border border-[#E5E7EB] flex items-center justify-center group-hover:border-[#F59E0B]/30 group-hover:shadow-sm transition-all">
                <PenTool className="w-4 h-4 text-[#F59E0B]" />
              </div>
              <div>
                <p className="text-[11px] font-bold text-[#111827] group-hover:text-[#F59E0B] transition-colors">AI Writer</p>
                <p className="text-[9px] text-[#6B7280]">Write blogs & articles</p>
              </div>
            </a>
            <a href="/tools/ai-article-outline-generator" className="flex items-center gap-3 p-2.5 bg-[#F9FAFB] hover:bg-[#FFFBEB] rounded-xl transition-colors group">
              <div className="w-8 h-8 shrink-0 rounded-lg bg-white border border-[#E5E7EB] flex items-center justify-center group-hover:border-[#F59E0B]/30 group-hover:shadow-sm transition-all">
                <LayoutGrid className="w-4 h-4 text-[#F59E0B]" />
              </div>
              <div>
                <p className="text-[11px] font-bold text-[#111827] group-hover:text-[#F59E0B] transition-colors">Outline Generator</p>
                <p className="text-[9px] text-[#6B7280]">Structure your posts</p>
              </div>
            </a>
          </div>
        </div>
        ...
        <main className="flex-grow flex flex-col min-w-0">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-2xl font-bold text-[#111827] flex items-center gap-2">
                    AI Blog Idea Generator <Sparkles className="w-5 h-5 text-[#F59E0B]" />
                  </h1>
                  <p className="text-sm text-[#6B7280]">Generate viral, highly engaging blog post ideas instantly.</p>
                </div>
              </div>
          ...
          {isProcessing ? (
            <div className="flex-grow bg-white rounded-3xl border border-[#E5E7EB] p-8 flex flex-col items-center justify-center shadow-sm animate-in fade-in duration-500 h-[600px]">
              ...
              <h2 className="text-xl font-bold text-[#111827]">Brainstorming Ideas...</h2>
              <p className="text-[#6B7280] mt-2">Finding the most engaging titles for your topic.</p>
            </div>
          ) : result ? (
            <div className="flex-grow flex flex-col bg-white border border-[#E5E7EB] rounded-3xl p-6 shadow-sm animate-in zoom-in-95 duration-500 h-[600px]">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-bold text-[#111827]">Blog Post Ideas</h2>
            ...
            <div className="flex-grow bg-white rounded-3xl border border-[#E5E7EB] p-8 lg:p-12 shadow-sm flex flex-col items-center justify-center text-center relative animate-in fade-in slide-in-from-bottom-8 duration-700 fill-mode-both delay-200 h-[600px]">
              ...
              <div className="relative w-72 h-64 mb-10 flex items-center justify-center mt-4">
                ...
                <div className="relative z-10 w-40 h-40 bg-white rounded-2xl shadow-[0_10px_30px_-10px_rgba(245,158,11,0.3)] border-4 border-amber-200 overflow-hidden flex items-center justify-center transform hover:scale-105 transition-transform duration-500 animate-[pulse_3s_infinite]">
                  <Lightbulb className="w-20 h-20 text-[#F59E0B]" />
                </div>

                <Sparkles className="absolute top-8 left-12 w-6 h-6 text-amber-300 animate-pulse" />
              ...
              <h2 className="text-2xl lg:text-3xl font-bold text-[#111827] mb-3">
                Need an <span className="text-[#F59E0B]">idea?</span>
              </h2>
              <p className="text-[#6B7280] max-w-sm mx-auto mb-8">
                Enter your niche or topic on the left and click <span className="text-[#F59E0B] font-semibold">"Generate Ideas"</span>.
              </p>
            </div>
          )}
        </main>
