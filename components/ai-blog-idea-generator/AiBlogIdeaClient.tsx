...
import { Download, Share2, RefreshCw, Lightbulb, History, Loader2, Copy, CheckCircle2, Sparkles, PenTool, LayoutGrid } from 'lucide-react';
...
export default function AiBlogIdeaClient() {
  ...
  const handleToggleFavorite = async (id: string) => {
    setToolHistory(prev => {
      ...
      if (!isAuthenticated) {
        localStorage.setItem('guestBlogIdeaHistory', JSON.stringify(updated));
      }
      return updated;
    });
    if (isAuthenticated) {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-8 h-full">
      ...
      {showHistory ? (
        ...
        <main className="flex-grow flex flex-col min-w-0">
          ...
          <div className="flex flex-col md:flex-row md:items-start lg:items-center justify-between gap-4 mb-6">
            <div>
              <h1 className="text-3xl lg:text-4xl font-bold tracking-tight text-[#111827] flex items-center gap-2">AI Blog Idea Generator <Sparkles className="w-6 h-6" style={{ color: '#F59E0B' }} /></h1>
              <p className="text-[#6B7280] text-sm lg:text-base mt-2">
                Generate 10 highly engaging and viral blog post ideas instantly.
              </p>
            </div>
          ...
          <div className="flex-grow bg-white border border-[#E5E7EB] rounded-3xl p-6 lg:p-8 shadow-sm flex flex-col h-[600px] animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both delay-150">
            {isProcessing ? (
              <TextGenerationProgress title="Brainstorming Ideas..." description="Finding the most engaging titles for your topic." />
            ) : result ? (
              <>
      ...
      <TextDownloadModal 
        ...
        toolSlug="ai-blog-idea-generator" 
