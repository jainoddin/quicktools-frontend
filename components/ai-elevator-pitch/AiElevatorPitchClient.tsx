...
import { Download, Mic, Mic2, History, Loader2, Copy, CheckCircle2, Sparkles, Target, RefreshCw, Share2, User, Building2 } from 'lucide-react';
...
export default function AiElevatorPitchClient() {
  ...
  const localStorageKey = 'guestAiElevatorPitchHistory';
  const toolSlug = '/tools/ai-elevator-pitch';
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch(getEndpoint('/api/tools/elevator-pitch'), {
        method: 'POST',
      ...
      if (data.success) {
        ...
        if (!isAuthenticated) {
          const historyItem = {
            ...
            contentType: 'Elevator Pitch Generator',
    ...
    <div className="flex flex-col lg:flex-row gap-8 h-full">
      ...
      <aside className="w-full lg:w-[340px] shrink-0 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both">
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="e.g., I am a freelance web developer helping local restaurants get online."
        ...
        <div className="pt-2">
          ...
          >
            {isProcessing ? (
              ...
              <><Mic2 className="w-5 h-5" /> Generate Pitch</>
            )}
          </button>
        </div>
        ...
        <div className="mt-4 pt-4 border-t border-[#E5E7EB]">
          ...
          <div className="flex flex-col gap-2">
            <a href="/tools/ai-linkedin-bio" className="flex items-center gap-3 p-2.5 bg-[#F9FAFB] hover:bg-[#0077B5]/5 rounded-xl transition-colors group">
              <div className="w-8 h-8 shrink-0 rounded-lg bg-white border border-[#E5E7EB] flex items-center justify-center group-hover:border-[#0077B5]/30 group-hover:shadow-sm transition-all">
                <User className="w-4 h-4" style={{color:'#0077B5'}} />
              </div>
              <div>
                <p className="text-[11px] font-bold text-[#111827] group-hover:text-[#0077B5] transition-colors">AI LinkedIn Bio</p>
                <p className="text-[9px] text-[#6B7280]">Write a powerful LinkedIn bio</p>
              </div>
            </a>
      ...
      {showHistory ? (
        <div className="flex-grow bg-white border border-[#E5E7EB] rounded-3xl p-6 shadow-sm overflow-y-auto h-[600px] animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both">
          <ToolHistorySidebar
            toolName="Elevator Pitch Generator"
            toolType="text"
            history={toolHistory}
        ...
        <main className="flex-grow flex flex-col min-w-0">
          ...
          <div className="flex flex-col md:flex-row md:items-start lg:items-center justify-between gap-4 mb-6">
            <div>
              <h1 className="text-3xl lg:text-4xl font-bold tracking-tight text-[#111827] flex items-center gap-2">AI Elevator Pitch Generator <Sparkles className="w-6 h-6" style={{ color: '#10B981' }} /></h1>
              <p className="text-[#6B7280] text-sm lg:text-base mt-2">
                Craft persuasive elevator pitches based on your role or product.
              </p>
            </div>
          ...
          <div className="flex-grow bg-white border border-[#E5E7EB] rounded-3xl p-6 lg:p-8 shadow-sm flex flex-col h-[600px] animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both delay-150">
            {isProcessing ? (
              <TextGenerationProgress title="Crafting Your Pitch..." description="Writing a persuasive and memorable elevator pitch." />
            ) : result ? (
              <>
                ...
                <div className="flex flex-wrap items-center gap-3 pt-6 border-t border-[#E5E7EB] shrink-0">
                  ...
                  <button
                    onClick={async () => {
                      if (navigator.share) {
                        try {
                          await navigator.share({ title: 'A I  Elevator Pitch', text: result });
                        } catch (err) {
      ...
      <TextDownloadModal 
        ...
        content={result} 
        filename="A I  Elevator Pitch" 
        toolSlug="ai-elevator-pitch" 
