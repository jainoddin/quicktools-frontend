...
export default function AiCourseOutlineClient() {
  ...
  useEffect(() => {
    if (!isAuthenticated) {
      const count = parseInt(localStorage.getItem('premium_ai-course-outline_count') || '0', 10);
      setFreeGenCount(count);
      const saved = localStorage.getItem('ai-course-outline_history');
      if (saved) {
      ...
      .then(data => {
        if (data.success && data.data?.history) {
          const items = data.data.history
            .filter((item: any) => item.toolSlug === '/tools/ai-course-outline')
            .map((item: any) => ({
              id: item._id,
              prompt: item.prompt,
              result: item.result,
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch(getEndpoint('/api/tools/generate-premium'), {
        ...
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({ input, toolSlug: 'ai-course-outline', toolName: 'AI Masterclass Course Outline' }),
      });
      ...
      if (data.success) {
        ...
        if (!isAuthenticated) {
          ...
          localStorage.setItem('premium_ai-course-outline_count', newCount.toString());
          ...
          localStorage.setItem('ai-course-outline_history', JSON.stringify(newHistory));
        } else {
    ...
    setToolHistory(prev => {
      ...
      if (!isAuthenticated) {
        localStorage.setItem('ai-course-outline_history', JSON.stringify(updated));
      }
      return updated;
    });
    if (isAuthenticated) {
    ...
    <div className="flex flex-col lg:flex-row gap-6 lg:h-full">
        ...
        <div className="bg-white border border-[#E5E7EB] rounded-3xl p-6 shadow-sm">
          <div className="mb-6">
            ...
            <p className="text-sm text-[#4B5563]">
              Generate detailed course outlines and curriculum.
            </p>
          </div>
      ...
      {showHistory ? (
        <div className="flex-grow bg-white border border-[#E5E7EB] rounded-3xl p-6 shadow-sm overflow-y-auto h-[600px] animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both">
          <ToolHistorySidebar
            toolName="AI Masterclass Course Outline"
            toolType="text"
            history={toolHistory}
              ...
              <>
                ...
                <div className="flex flex-wrap items-center gap-3 pt-6 border-t border-[#E5E7EB] shrink-0">
                  ...
                  <button
                    onClick={async () => {
                      if (navigator.share) {
                        try { await navigator.share({ title: 'AI Masterclass Course Outline', text: result }); } catch (err) {}
      ...
      <TextDownloadModal 
        ...
        content={result} 
        filename="AI Masterclass Course Outline" 
        toolSlug="ai-course-outline" 
