...
import { CheckCircle2, Loader2, Circle, PenTool, Palette, Sparkles } from 'lucide-react';
  ...
  return (
    <div className="flex-grow min-h-[500px] bg-white rounded-2xl border border-[#E5E7EB] flex flex-col items-center p-8 shadow-sm relative overflow-hidden">
      ...
      <div className="text-center mb-8 mt-4">
        <h2 className="text-2xl font-bold text-[#111827] mb-2">Generating your image...</h2>
      ...
      <div className="grid md:grid-cols-2 gap-8 w-full max-w-2xl mb-8 items-start">
        ...
        <div className="flex flex-col gap-6 relative">
          ...
          <StepItem active={currentStep >= 1} completed={currentStep > 1} text="Understanding your prompt" />
          <StepItem active={currentStep >= 2} completed={currentStep > 2} text="Choosing the best style" />
          <StepItem active={currentStep >= 3} completed={currentStep > 3} text="Creating your image" />
          <StepItem active={currentStep >= 4} completed={currentStep > 4} text="Finalizing and enhancing" />
        </div>

        {/* Right: Tips */}
        <div className="flex flex-col gap-4">
          <TipCard icon={PenTool} text="More details in your prompt = better results." />
          <TipCard icon={Palette} text="Try different styles for unique results." />
          <TipCard icon={Sparkles} text="High quality images take a little more time." />
        </div>
      </div>
