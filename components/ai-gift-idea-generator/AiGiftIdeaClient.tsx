...
import { Download, Share2, RefreshCw, Gift, History, Loader2, Copy, CheckCircle2, Sparkles, Coffee } from 'lucide-react';
...
export default function AiGiftIdeaClient() {
  ...
  const handleToggleFavorite = async (id: string) => {
    setToolHistory(prev => {
      ...
      if (!isAuthenticated) {
        localStorage.setItem('guestGiftHistory', JSON.stringify(updated));
      }
      return updated;
    });
    if (isAuthenticated) {
      ...
      {showHistory ? (
        ...
        <main className="flex-grow flex flex-col min-w-0">
          ...
          <div className="flex flex-col md:flex-row md:items-start lg:items-center justify-between gap-4 mb-6">
            <div>
              <h1 className="text-3xl lg:text-4xl font-bold tracking-tight text-[#111827] flex items-center gap-2">AI Gift Idea Generator <Sparkles className="w-6 h-6" style={{ color: '#EC4899' }} /></h1>
              <p className="text-[#6B7280] text-sm lg:text-base mt-2">
                Find the perfect, thoughtful gift based on age and interests.
              </p>
            </div>
          ...
          <div className="flex-grow bg-white border border-[#E5E7EB] rounded-3xl p-6 lg:p-8 shadow-sm flex flex-col h-[600px] animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both delay-150">
            {isProcessing ? (
              <TextGenerationProgress title="Finding Gifts..." description="Matching interests with unique ideas." />
            ) : result ? (
              <>
