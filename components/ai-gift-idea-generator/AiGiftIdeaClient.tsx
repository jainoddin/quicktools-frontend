...
import { 
  Gift, History, Loader2, Copy, CheckCircle2,
...
export default function AiGiftIdeaClient() {
  ...
  useEffect(() => {
    if (!isAuthenticated) {
      const saved = localStorage.getItem('guestGiftHistory');
  ...
  const handleGenerate = async () => {
    if (!interests.trim() || !age.trim()) return;
    setIsProcessing(true);
    setResult('');
    
    try {
      const res = await fetch(getEndpoint('/api/tools/gift-idea'), {
        method: 'POST',
      ...
      if (data.success) {
        ...
        const promptString = `${age} yo, ${interests.substring(0, 50)}`;
        if (!isAuthenticated) {
          const historyItem = {
            id: data.usageId || Date.now().toString(),
            prompt: promptString,
            contentType: 'Gift Ideas',
          ...
          localStorage.setItem('guestGiftHistory', JSON.stringify(newHistory));
        } else {
          setToolHistory(prev => [{
            id: data.usageId || Date.now().toString(),
            prompt: promptString,
    ...
    <div className="flex flex-col lg:flex-row gap-8 h-full">
      ...
      <aside className="w-full lg:w-[340px] shrink-0 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both">
        ...
        <div className="space-y-4">
          <div>
            <label className="text-sm font-bold text-[#111827] mb-2 block">Recipient's Age</label>
            <input
              type="number"
              value={age}
              onChange={(e) => setAge(e.target.value)}
              placeholder="e.g. 25"
              className="w-full p-3 bg-white rounded-xl border border-[#E5E7EB] text-sm text-[#111827] focus:outline-none focus:ring-2 focus:ring-[#EC4899]/20 focus:border-[#EC4899] transition-all shadow-sm"
            />
          </div>

          <div>
            <label className="flex items-center justify-between mb-2">
              <span className="text-sm font-bold text-[#111827]">Their Interests/Hobbies</span>
              <span className="text-xs text-[#6B7280]">{interests.length}/200</span>
            </label>
            <textarea
              value={interests}
              onChange={(e) => setInterests(e.target.value.substring(0, 200))}
              placeholder="e.g., Reading, traveling, cooking, tech gadgets, coffee..."
              className="w-full h-32 p-4 bg-white rounded-2xl border border-[#E5E7EB] text-sm text-[#4B5563] placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#EC4899]/20 focus:border-[#EC4899] transition-all resize-none shadow-sm"
            />
          </div>
        </div>
        ...
        <div className="pt-2">
          <button
            ...
            disabled={isProcessing || !interests.trim() || !age.trim()}
            className={`w-full text-white font-bold py-3.5 rounded-xl transition-all shadow-md hover:shadow-lg flex items-center justify-center gap-2 ${isProcessing || !interests.trim() || !age.trim() ? 'bg-[#9CA3AF] cursor-not-allowed' : 'bg-[#EC4899] hover:bg-[#DB2777]'}`}
          >
            {isProcessing ? (
              <><Loader2 className="w-5 h-5 animate-spin" /> Brainstorming...</>
            ) : (
              <><Gift className="w-5 h-5" /> Find Gift Ideas</>
            )}
          </button>
        </div>
        ...
        <div className="mt-4 pt-4 border-t border-[#E5E7EB]">
          ...
          <div className="flex flex-col gap-2">
            <a href="/tools/ai-recipe-generator" className="flex items-center gap-3 p-2.5 bg-[#F9FAFB] hover:bg-[#FDF2F8] rounded-xl transition-colors group">
              ...
              <div>
                <p className="text-[11px] font-bold text-[#111827] group-hover:text-[#EC4899] transition-colors">Recipe Generator</p>
        ...
        <main className="flex-grow flex flex-col min-w-0">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-2xl font-bold text-[#111827] flex items-center gap-2">
                    AI Gift Idea Generator <Sparkles className="w-5 h-5 text-[#EC4899]" />
                  </h1>
                  <p className="text-sm text-[#6B7280]">Find the perfect, thoughtful gift for anyone instantly.</p>
                </div>
              </div>
          ...
          {isProcessing ? (
            <div className="flex-grow bg-white rounded-3xl border border-[#E5E7EB] p-8 flex flex-col items-center justify-center shadow-sm animate-in fade-in duration-500 h-[600px]">
              ...
              <h2 className="text-xl font-bold text-[#111827]">Finding Gifts...</h2>
              <p className="text-[#6B7280] mt-2">Matching interests with unique ideas.</p>
            </div>
          ) : result ? (
            <div className="flex-grow flex flex-col bg-white border border-[#E5E7EB] rounded-3xl p-6 shadow-sm animate-in zoom-in-95 duration-500 h-[600px]">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-bold text-[#111827]">Gift Ideas</h2>
            ...
            <div className="flex-grow bg-white rounded-3xl border border-[#E5E7EB] p-8 lg:p-12 shadow-sm flex flex-col items-center justify-center text-center relative animate-in fade-in slide-in-from-bottom-8 duration-700 fill-mode-both delay-200 h-[600px]">
              ...
              <div className="relative w-72 h-64 mb-10 flex items-center justify-center mt-4">
                ...
                <div className="relative z-10 w-44 h-32 bg-white rounded-xl shadow-[0_10px_30px_-10px_rgba(236,72,153,0.3)] border-4 border-pink-200 overflow-hidden flex flex-col items-center justify-center transform hover:scale-110 transition-transform duration-500">
                  <Gift className="w-16 h-16 text-[#EC4899] drop-shadow-md animate-[pulse_2s_infinite]" />
                </div>
              ...
              <h2 className="text-2xl lg:text-3xl font-bold text-[#111827] mb-3">
                Need a <span className="text-[#EC4899]">gift?</span>
              </h2>
              <p className="text-[#6B7280] max-w-sm mx-auto mb-8">
                Tell us about them on the left and click <span className="text-[#EC4899] font-semibold">"Find Gift Ideas"</span>.
              </p>
            </div>
          )}
        </main>
