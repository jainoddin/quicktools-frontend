...
import { Download, Wind, History, Loader2, Copy, CheckCircle2, Sparkles, Target, RefreshCw, Share2, BookOpen, Quote } from 'lucide-react';
          ...
          <div className="flex flex-col gap-2">
            <a href="/tools/ai-story-generator" className="flex items-center gap-3 p-2.5 bg-[#F9FAFB] hover:bg-[#6D5EF8]/5 rounded-xl transition-colors group">
              <div className="w-8 h-8 shrink-0 rounded-lg bg-white border border-[#E5E7EB] flex items-center justify-center group-hover:border-[#6D5EF8]/30 group-hover:shadow-sm transition-all">
                <BookOpen className="w-4 h-4" style={{color:'#6D5EF8'}} />
              </div>
              <div>
            ...
            <a href="/tools/ai-quote-generator" className="flex items-center gap-3 p-2.5 bg-[#F9FAFB] hover:bg-[#F59E0B]/5 rounded-xl transition-colors group">
              <div className="w-8 h-8 shrink-0 rounded-lg bg-white border border-[#E5E7EB] flex items-center justify-center group-hover:border-[#F59E0B]/30 group-hover:shadow-sm transition-all">
                <Quote className="w-4 h-4" style={{color:'#F59E0B'}} />
              </div>
              <div>
                <p className="text-[11px] font-bold text-[#111827] group-hover:text-[#F59E0B] transition-colors">AI Quote Generator</p>
                <p className="text-[9px] text-[#6B7280]">Generate inspiring quotes</p>
              </div>
            </a>
          </div>
        </div>
      ...
      {showHistory ? (
        ...
        <main className="flex-grow flex flex-col min-w-0">
          ...
          <div className="flex flex-col md:flex-row md:items-start lg:items-center justify-between gap-4 mb-6">
            <div>
              <h1 className="text-3xl lg:text-4xl font-bold tracking-tight text-[#111827] flex items-center gap-2">AI Poem Generator <Sparkles className="w-6 h-6" style={{ color: '#A855F7' }} /></h1>
              <p className="text-[#6B7280] text-sm lg:text-base mt-2">
                Generate beautiful poems in various styles and formats.
              </p>
            </div>
          ...
          <div className="flex-grow bg-white border border-[#E5E7EB] rounded-3xl p-6 lg:p-8 shadow-sm flex flex-col h-[600px] animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both delay-150">
            {isProcessing ? (
              <TextGenerationProgress title="Writing Your Poem..." description="Weaving words into a beautiful poem." />
            ) : result ? (
              <>
                ...
                <div className="flex flex-wrap items-center gap-3 pt-6 border-t border-[#E5E7EB] shrink-0">
                  ...
                  <button
                    onClick={async () => {
                      if (navigator.share) {
                        try {
                          await navigator.share({ title: 'A I  Poem Generator', text: result });
                        } catch (err) {
