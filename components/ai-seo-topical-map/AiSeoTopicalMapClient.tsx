...
import { Map, Sparkles, Copy, CheckCircle2, Download, History, Loader2, Crown, Share2, RefreshCw } from 'lucide-react';
...
export default function AiSeoTopicalMapClient() {
  ...
  useEffect(() => {
    if (!isAuthenticated) {
      const count = parseInt(localStorage.getItem('premium_ai-seo-topical-map_count') || '0', 10);
      setFreeGenCount(count);
      const saved = localStorage.getItem('ai-seo-topical-map_history');
      if (saved) {
      ...
      .then(data => {
        if (data.success && data.data?.history) {
          const items = data.data.history
            .filter((item: any) => item.toolSlug === '/tools/ai-seo-topical-map')
            .map((item: any) => ({
              id: item._id,
              prompt: item.prompt,
              result: item.result,
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch(getEndpoint('/api/tools/ai-seo-topical-map'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
      ...
      if (data.success) {
        ...
        if (!isAuthenticated) {
          ...
          localStorage.setItem('premium_ai-seo-topical-map_count', newCount.toString());
          ...
          localStorage.setItem('ai-seo-topical-map_history', JSON.stringify(newHistory));
        } else {
    ...
    setToolHistory(prev => {
      ...
      if (!isAuthenticated) {
        localStorage.setItem('ai-seo-topical-map_history', JSON.stringify(updated));
      }
      return updated;
    });
    if (isAuthenticated) {
    ...
    <div className="flex flex-col lg:flex-row gap-6 lg:h-full">
        ...
        <div className="bg-white border border-[#E5E7EB] rounded-3xl p-6 shadow-sm">
          <div className="mb-6">
            ...
            <p className="text-sm text-[#4B5563]">
              Generate a full SEO content cluster map for an entire month for a niche.
            </p>
          </div>
      ...
      {showHistory ? (
        <div className="flex-grow bg-white border border-[#E5E7EB] rounded-3xl p-6 shadow-sm overflow-y-auto h-[600px] animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both">
          <ToolHistorySidebar
            toolName="AI SEO Topical Map Builder"
            toolType="text"
            history={toolHistory}
        ...
        <main className="flex-grow flex flex-col min-w-0">
          ...
          <div className="flex flex-col md:flex-row md:items-start lg:items-center justify-between gap-4 mb-6">
            <div>
              <h1 className="text-3xl lg:text-4xl font-bold tracking-tight text-[#111827] flex items-center gap-2">AI SEO Topical Map Builder <Sparkles className="w-6 h-6" style={{ color: '#06B6D4' }} /></h1>
                ...
                <div className="flex flex-wrap items-center gap-3 pt-6 border-t border-[#E5E7EB] shrink-0">
                  ...
                  <button
                    onClick={async () => {
                      if (navigator.share) {
                        try {
                          await navigator.share({ title: 'AI SEO Topical Map Builder', text: result });
                        } catch (err) {
      ...
      <TextDownloadModal 
        ...
        content={result} 
        filename="AI SEO Topical Map Builder" 
        toolSlug="ai-seo-topical-map" 
