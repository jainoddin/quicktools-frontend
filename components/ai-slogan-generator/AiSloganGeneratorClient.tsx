...
import { Download, Megaphone, History, Loader2, Copy, CheckCircle2, Sparkles, Target, RefreshCw, Share2, Building2 } from 'lucide-react';
...
export default function AiSloganGeneratorClient() {
  ...
  const localStorageKey = 'guestAiSloganGeneratorHistory';
  const toolSlug = '/tools/ai-slogan-generator';
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch(getEndpoint('/api/tools/slogan-generator'), {
        method: 'POST',
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-8 h-full">
      ...
      <aside className="w-full lg:w-[340px] shrink-0 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both">
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="e.g., An eco-friendly coffee brand that sources fair-trade beans"
        ...
        <div className="pt-2">
          <button
            ...
            className={`w-full text-white font-bold py-3.5 rounded-xl transition-all shadow-md hover:shadow-lg flex items-center justify-center gap-2 ${isProcessing || !inputVal.trim() ? 'bg-[#9CA3AF] cursor-not-allowed' : 'bg-[#F43F5E] hover:opacity-90'}`}
          >
            {isProcessing ? (
              <><Loader2 className="w-5 h-5 animate-spin" /> Generating...</>
            ) : (
              <><Megaphone className="w-5 h-5" /> Generate Slogans</>
            )}
          </button>
        </div>
      ...
      {showHistory ? (
        ...
        <main className="flex-grow flex flex-col min-w-0">
          ...
          <div className="flex flex-col md:flex-row md:items-start lg:items-center justify-between gap-4 mb-6">
            <div>
              <h1 className="text-3xl lg:text-4xl font-bold tracking-tight text-[#111827] flex items-center gap-2">AI Slogan Generator <Sparkles className="w-6 h-6" style={{ color: '#F43F5E' }} /></h1>
              <p className="text-[#6B7280] text-sm lg:text-base mt-2">
                Create catchy and memorable slogans for your brand or campaign.
              </p>
            </div>
          ...
          {result && !isProcessing && (
            <div className="flex items-center justify-between mb-4 mt-2">
              <h2 className="text-xl font-extrabold text-[#111827] flex items-center gap-2">
                <Sparkles className="w-6 h-6" style={{ color: '#F43F5E' }} />
                Generated Result
              </h2>
          ...
          <div className="flex-grow bg-white border border-[#E5E7EB] rounded-3xl p-6 lg:p-8 shadow-sm flex flex-col h-[600px] animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both delay-150">
            {isProcessing ? (
              <TextGenerationProgress title="Generating Slogans..." description="Coming up with catchy and memorable slogans." />
            ) : result ? (
              <>
                ...
                <div className="flex flex-wrap items-center gap-3 pt-6 border-t border-[#E5E7EB] shrink-0">
                  ...
                  <button
                    onClick={async () => {
                      if (navigator.share) {
                        try {
                          await navigator.share({ title: 'A I  Slogan Generator', text: result });
                        } catch (err) {
      ...
      <TextDownloadModal 
        ...
        content={result} 
        filename="A I  Slogan Generator" 
        toolSlug="ai-slogan-generator" 
