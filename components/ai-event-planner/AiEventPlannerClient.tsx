...
import { Download, Calendar, History, Loader2, Copy, CheckCircle2, Sparkles, Target, RefreshCw, Share2, Plane, UtensilsCrossed } from 'lucide-react';
...
export default function AiEventPlannerClient() {
  ...
  const localStorageKey = 'guestAiEventPlannerHistory';
  const toolSlug = '/tools/ai-event-planner';
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch(getEndpoint('/api/tools/event-planner'), {
        method: 'POST',
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-8 h-full">
      ...
      <aside className="w-full lg:w-[340px] shrink-0 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both">
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="e.g., A surprise 30th birthday party for 50 people on a budget"
            className="w-full h-40 p-4 bg-white rounded-2xl border border-[#E5E7EB] text-sm text-[#4B5563] placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#84CC16]/20 focus:border-[#84CC16] transition-all resize-none shadow-sm"
          />
        </div>
        ...
        <div className="pt-2">
          <button
            ...
            className={`w-full text-white font-bold py-3.5 rounded-xl transition-all shadow-md hover:shadow-lg flex items-center justify-center gap-2 ${isProcessing || !inputVal.trim() ? 'bg-[#9CA3AF] cursor-not-allowed' : 'bg-[#84CC16] hover:opacity-90'}`}
          >
            {isProcessing ? (
              <><Loader2 className="w-5 h-5 animate-spin" /> Generating...</>
            ) : (
              <><Calendar className="w-5 h-5" /> Generate Event Plan</>
            )}
          </button>
        </div>
        ...
        <div className="mt-4 pt-4 border-t border-[#E5E7EB]">
          ...
          <div className="flex flex-col gap-2">
            <a href="/tools/ai-travel-planner" className="flex items-center gap-3 p-2.5 bg-[#F9FAFB] hover:bg-[#0EA5E9]/5 rounded-xl transition-colors group">
              ...
              <div>
                <p className="text-[11px] font-bold text-[#111827] group-hover:text-[#0EA5E9] transition-colors">AI Travel Planner</p>
                <p className="text-[9px] text-[#6B7280]">Plan your perfect trip</p>
              </div>
            </a>
            <a href="/tools/ai-meal-planner" className="flex items-center gap-3 p-2.5 bg-[#F9FAFB] hover:bg-[#22C55E]/5 rounded-xl transition-colors group">
              <div className="w-8 h-8 shrink-0 rounded-lg bg-white border border-[#E5E7EB] flex items-center justify-center group-hover:border-[#22C55E]/30 group-hover:shadow-sm transition-all">
                <UtensilsCrossed className="w-4 h-4" style={{color:'#22C55E'}} />
              </div>
              <div>
                <p className="text-[11px] font-bold text-[#111827] group-hover:text-[#22C55E] transition-colors">AI Meal Planner</p>
                <p className="text-[9px] text-[#6B7280]">Create healthy meal plans</p>
              </div>
            </a>
          </div>
        </div>
      ...
      {showHistory ? (
        ...
        <main className="flex-grow flex flex-col min-w-0">
          ...
          <div className="flex flex-col md:flex-row md:items-start lg:items-center justify-between gap-4 mb-6">
            <div>
              <h1 className="text-3xl lg:text-4xl font-bold tracking-tight text-[#111827] flex items-center gap-2">AI Event Planner <Sparkles className="w-6 h-6" style={{ color: '#84CC16' }} /></h1>
              <p className="text-[#6B7280] text-sm lg:text-base mt-2">
                Generate a step-by-step plan and schedule for any event or party.
              </p>
            </div>
          ...
          {result && !isProcessing && (
            <div className="flex items-center justify-between mb-4 mt-2">
              <h2 className="text-xl font-extrabold text-[#111827] flex items-center gap-2">
                <Sparkles className="w-6 h-6" style={{ color: '#84CC16' }} />
                Generated Result
              </h2>
          ...
          <div className="flex-grow bg-white border border-[#E5E7EB] rounded-3xl p-6 lg:p-8 shadow-sm flex flex-col h-[600px] animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both delay-150">
            {isProcessing ? (
              <TextGenerationProgress title="Planning Your Event..." description="Organizing every detail of your perfect event." />
            ) : result ? (
              <>
                ...
                <div className="flex flex-wrap items-center gap-3 pt-6 border-t border-[#E5E7EB] shrink-0">
                  ...
                  <button
                    onClick={async () => {
                      if (navigator.share) {
                        try {
                          await navigator.share({ title: 'A I  Event Planner', text: result });
                        } catch (err) {
      ...
      <TextDownloadModal 
        ...
        content={result} 
        filename="A I  Event Planner" 
        toolSlug="ai-event-planner" 
