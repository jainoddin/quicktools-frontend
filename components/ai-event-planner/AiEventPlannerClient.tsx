...
import { Download, Calendar, History, Loader2, Copy, CheckCircle2, Sparkles, Target } from 'lucide-react';
          ...
          {!result && !isProcessing && (
            <div className="flex flex-col md:flex-row md:items-start lg:items-center justify-between gap-4 mb-6 animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both delay-100">
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 bg-[#84CC16] rounded-xl flex items-center justify-center shadow-sm shrink-0">
                  <Calendar className="w-6 h-6 text-white" />
                </div>
                <div>
                  <h1 className="text-2xl font-bold text-[#111827] flex items-center gap-2">
                    AI Event Planner <Sparkles className="w-5 h-5 text-[#84CC16]" />
                  </h1>
                  <p className="text-sm text-[#6B7280]">Generate a step-by-step plan and schedule for any event or party.</p>
                </div>
              </div>
            ...
            <div className="flex gap-2">
              {result && (
                <>
                  ...
                  <button
                    onClick={() => downloadAsPDF(result, 'A I  Event Planner')}
