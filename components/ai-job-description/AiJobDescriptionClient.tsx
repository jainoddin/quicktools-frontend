...
import { Download, Briefcase, History, Loader2, Copy, CheckCircle2, Sparkles, Target, RefreshCw, Share2, FileText } from 'lucide-react';
...
export default function AiJobDescriptionClient() {
  ...
  const localStorageKey = 'guestAiJobDescriptionHistory';
  const toolSlug = '/tools/ai-job-description';
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch(getEndpoint('/api/tools/job-description'), {
        method: 'POST',
      ...
      if (data.success) {
        ...
        if (!isAuthenticated) {
          const historyItem = {
            ...
            contentType: 'Job Description Generator',
    ...
    <div className="flex flex-col lg:flex-row gap-8 h-full">
      ...
      <aside className="w-full lg:w-[340px] shrink-0 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both">
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="e.g., Marketing Manager, 3+ years experience, remote, focus on SEO and social media"
        ...
        <div className="pt-2">
          <button
            ...
            className={`w-full text-white font-bold py-3.5 rounded-xl transition-all shadow-md hover:shadow-lg flex items-center justify-center gap-2 ${isProcessing || !inputVal.trim() ? 'bg-[#9CA3AF] cursor-not-allowed' : 'bg-[#F59E0B] hover:opacity-90'}`}
          >
            {isProcessing ? (
              <><Loader2 className="w-5 h-5 animate-spin" /> Generating...</>
            ) : (
              <><Briefcase className="w-5 h-5" /> Generate Description</>
            )}
          </button>
        </div>
        ...
        <div className="mt-4 pt-4 border-t border-[#E5E7EB]">
          ...
          <div className="flex flex-col gap-2">
            <a href="/tools/ai-interview-questions" className="flex items-center gap-3 p-2.5 bg-[#F9FAFB] hover:bg-[#3B82F6]/5 rounded-xl transition-colors group">
              ...
              <div>
                <p className="text-[11px] font-bold text-[#111827] group-hover:text-[#3B82F6] transition-colors">AI Interview Questions</p>
                <p className="text-[9px] text-[#6B7280]">Generate role-specific questions</p>
              </div>
            </a>
            <a href="/tools/ai-resume-builder" className="flex items-center gap-3 p-2.5 bg-[#F9FAFB] hover:bg-[#6D5EF8]/5 rounded-xl transition-colors group">
              <div className="w-8 h-8 shrink-0 rounded-lg bg-white border border-[#E5E7EB] flex items-center justify-center group-hover:border-[#6D5EF8]/30 group-hover:shadow-sm transition-all">
                <Sparkles className="w-4 h-4" style={{color:'#6D5EF8'}} />
              </div>
              <div>
      ...
      {showHistory ? (
        <div className="flex-grow bg-white border border-[#E5E7EB] rounded-3xl p-6 shadow-sm overflow-y-auto h-[600px] animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both">
          <ToolHistorySidebar
            toolName="Job Description Generator"
            toolType="text"
            history={toolHistory}
        ...
        <main className="flex-grow flex flex-col min-w-0">
          ...
          <div className="flex flex-col md:flex-row md:items-start lg:items-center justify-between gap-4 mb-6">
            <div>
              <h1 className="text-3xl lg:text-4xl font-bold tracking-tight text-[#111827] flex items-center gap-2">AI Job Description Generator <Sparkles className="w-6 h-6" style={{ color: '#F59E0B' }} /></h1>
              <p className="text-[#6B7280] text-sm lg:text-base mt-2">
                Create comprehensive and professional job descriptions for hiring.
              </p>
            </div>
          ...
          <div className="flex-grow bg-white border border-[#E5E7EB] rounded-3xl p-6 lg:p-8 shadow-sm flex flex-col h-[600px] animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both delay-150">
            {isProcessing ? (
              <TextGenerationProgress title="Writing Job Description..." description="Crafting a detailed and attractive job posting." />
            ) : result ? (
              <>
                ...
                <div className="flex flex-wrap items-center gap-3 pt-6 border-t border-[#E5E7EB] shrink-0">
                  ...
                  <button
                    onClick={async () => {
                      if (navigator.share) {
                        try {
                          await navigator.share({ title: 'A I  Job Description', text: result });
                        } catch (err) {
      ...
      <TextDownloadModal 
        ...
        content={result} 
        filename="A I  Job Description" 
        toolSlug="ai-job-description" 
