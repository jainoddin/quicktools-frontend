...
import { Download, PenTool, History, Loader2, Copy, CheckCircle2, Sparkles, Target, RefreshCw, Share2, Mail, FileText } from 'lucide-react';
...
export default function AiApologyLetterClient() {
  ...
  const localStorageKey = 'guestAiApologyLetterHistory';
  const toolSlug = '/tools/ai-apology-letter';
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch(getEndpoint('/api/tools/apology-letter'), {
        method: 'POST',
      ...
      if (data.success) {
        ...
        if (!isAuthenticated) {
          const historyItem = {
            ...
            contentType: 'Apology Letter Generator',
    ...
    <div className="flex flex-col lg:flex-row gap-8 h-full">
      ...
      <aside className="w-full lg:w-[340px] shrink-0 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both">
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="e.g., I missed an important client meeting because I overslept"
        ...
        <div className="pt-2">
          ...
          >
            {isProcessing ? (
              ...
              <><PenTool className="w-5 h-5" /> Generate Letter</>
            )}
          </button>
        </div>
      ...
      {showHistory ? (
        <div className="flex-grow bg-white border border-[#E5E7EB] rounded-3xl p-6 shadow-sm overflow-y-auto h-[600px] animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both">
          <ToolHistorySidebar
            toolName="Apology Letter Generator"
            toolType="text"
            history={toolHistory}
        ...
        <main className="flex-grow flex flex-col min-w-0">
          ...
          <div className="flex flex-col md:flex-row md:items-start lg:items-center justify-between gap-4 mb-6">
            <div>
              <h1 className="text-3xl lg:text-4xl font-bold tracking-tight text-[#111827] flex items-center gap-2">AI Apology Letter Generator <Sparkles className="w-6 h-6" style={{ color: '#64748B' }} /></h1>
              <p className="text-[#6B7280] text-sm lg:text-base mt-2">
                Write sincere, professional, or personal apology letters.
              </p>
            </div>
          ...
          <div className="flex-grow bg-white border border-[#E5E7EB] rounded-3xl p-6 lg:p-8 shadow-sm flex flex-col h-[600px] animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both delay-150">
            {isProcessing ? (
              <TextGenerationProgress title="Writing Your Apology..." description="Crafting a sincere and heartfelt apology letter." />
            ) : result ? (
              <>
                ...
                <div className="flex flex-wrap items-center gap-3 pt-6 border-t border-[#E5E7EB] shrink-0">
                  ...
                  <button
                    onClick={async () => {
                      if (navigator.share) {
                        try {
                          await navigator.share({ title: 'A I  Apology Letter', text: result });
                        } catch (err) {
      ...
      <TextDownloadModal 
        ...
        content={result} 
        filename="A I  Apology Letter" 
        toolSlug="ai-apology-letter" 
