...
import { Search, Star, Filter, Trash2, Eye, Download, Heart, ChevronLeft, ChevronRight, ChevronDown, Crown } from 'lucide-react';
...
export default function HistoryView({ onClose, isAuthenticated = false, onRequireLogin, recentImages = [] }: HistoryViewProps) {
  ...
  useEffect(() => {
    const savedDeleted = localStorage.getItem('quicktools_deleted_bg_images');
    if (savedDeleted) {
      try {
        setDeletedIds(new Set(JSON.parse(savedDeleted)));
      } catch (e) {
        console.error("Failed to parse deleted bg images", e);
      }
    }
  }, []);
  ...
  useEffect(() => {
    if (isAuthenticated) {
      ...
      .then(data => {
        if (data.success) {
          const bgFavs = data.data.filter((img: any) => img.toolSlug === '/tools/background-remover');
          setBackendFavorites(bgFavs);
          const favIds = new Set<number>(bgFavs.map((img: any) => img.id as number));
          setFavorites(favIds);
        }
      })
      .catch(err => console.error('Failed to fetch bg favorites:', err));
    }
  }, [isAuthenticated]);
  ...
  const toggleFavorite = async (img: any) => {
    ...
    if (isAuthenticated) {
      try {
        ...
        if (data.success) {
          const bgFavs = data.data.filter((item: any) => item.toolSlug === '/tools/background-remover');
          setBackendFavorites(bgFavs);
        }
      } catch (err) {
        console.error('Failed to toggle bg favorite:', err);
      }
    }
  };
  ...
  const handleDeleteSelected = async () => {
    ...
    setDeletedIds(prev => {
      ...
      localStorage.setItem('quicktools_deleted_bg_images', JSON.stringify(Array.from(next)));
      return next;
    });
    ...
    if (isAuthenticated) {
      try {
        ...
        console.error('Failed to delete bg usage from backend', e);
      }
    }
      ...
      {filteredImages.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 px-4 border border-dashed border-gray-200 rounded-2xl bg-gray-50">
          ...
          <p className="text-[#6B7280] text-center max-w-sm mb-6">
            {allImages.length === 0 
              ? "You haven't processed any images yet. Go back to the tool to remove your first background!" 
          ...
          {paginatedImages.map((img) => (
            <div key={img.id} className="bg-white rounded-2xl border border-[#E5E7EB] overflow-hidden shadow-sm hover:shadow-md hover:border-gray-300 transition-all group flex flex-col">
              ...
              <div 
                ...
                style={{ backgroundImage: 'conic-gradient(#E5E7EB 90deg, #FFFFFF 90deg 180deg, #E5E7EB 180deg 270deg, #FFFFFF 270deg)', backgroundSize: '16px 16px' }}
                onContextMenu={(e) => {
                  if (!isAuthenticated) {
                    e.preventDefault();
                    onRequireLogin?.();
              ...
              >
                <Image 
                  ...
                  alt={img.filename} 
                  fill 
                  className={`object-contain group-hover:scale-105 transition-transform duration-500 ${!isAuthenticated ? 'filter blur-[3px] select-none pointer-events-none' : ''}`} 
                    ...
                    <button 
                      onClick={() => {
                        if (!isAuthenticated && onRequireLogin) {
                          ...
                          downloadImage(img.src, img.filename);
                        }
                      }} 
