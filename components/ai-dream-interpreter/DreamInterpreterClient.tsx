...
import { Download, History, Loader2, Copy, CheckCircle2, Sparkles, RefreshCw, Share2, Building2, Megaphone, CloudSun } from 'lucide-react';
...
export default function DreamInterpreterClient() {
  ...
  const localStorageKey = 'guest_ai-dream-interpreter_history';
          ...
          if (data.success && data.data?.history) {
            const items = data.data.history
              ...
              .map((item: any) => ({
                ...
                prompt: item.prompt,
                contentType: 'AI Dream Interpreter',
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch(getEndpoint('/api/tools/ai-dream-interpreter'), {
        method: 'POST',
      ...
      <aside className="w-full lg:w-[340px] shrink-0 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both">
        ...
        <div className="pt-2">
          <button
            ...
            style={{ backgroundColor: isProcessing || !inputVal.trim() ? '' : '#8B5CF6' }}
          >
            {isProcessing ? (
              <><Loader2 className="w-5 h-5 animate-spin" /> Generating...</>
            ) : (
              <><CloudSun className="w-5 h-5" /> Generate Results</>
            )}
          </button>
        </div>
      ...
      {showHistory ? (
        ...
        <main className="flex-grow flex flex-col min-w-0">
          ...
          <div className="flex flex-col md:flex-row md:items-start lg:items-center justify-between gap-4 mb-6">
            <div>
              <h1 className="text-3xl lg:text-4xl font-bold tracking-tight text-[#111827] flex items-center gap-2">AI Dream Interpreter <Sparkles className="w-6 h-6" style={{ color: '#8B5CF6' }} /></h1>
              <p className="text-[#6B7280] text-sm lg:text-base mt-2">
                Analyze your dreams for psychological meaning and hidden messages.
              </p>
            </div>
                ...
                <div className="flex flex-wrap items-center gap-3 pt-6 border-t border-[#E5E7EB] shrink-0">
                  ...
                  <button
                    onClick={async () => {
                      if (navigator.share) {
                        try {
                          await navigator.share({ title: 'AI Dream Interpreter', text: result });
                        } catch (err) {
