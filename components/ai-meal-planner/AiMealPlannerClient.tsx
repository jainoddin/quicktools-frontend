...
import { Download, Utensils, History, Loader2, Copy, CheckCircle2, Sparkles, Target, RefreshCw, Share2, ChefHat, Dumbbell } from 'lucide-react';
...
export default function AiMealPlannerClient() {
  ...
  const localStorageKey = 'guestAiMealPlannerHistory';
  const toolSlug = '/tools/ai-meal-planner';
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch(getEndpoint('/api/tools/meal-planner'), {
        method: 'POST',
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-8 h-full">
      ...
      <aside className="w-full lg:w-[340px] shrink-0 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both">
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="e.g., Vegetarian, high protein, 2000 calories a day"
        ...
        <div className="pt-2">
          ...
          >
            {isProcessing ? (
              ...
              <><Utensils className="w-5 h-5" /> Generate Meal Plan</>
            )}
          </button>
        </div>
        ...
        <div className="mt-4 pt-4 border-t border-[#E5E7EB]">
          ...
          <div className="flex flex-col gap-2">
            <a href="/tools/ai-recipe-generator" className="flex items-center gap-3 p-2.5 bg-[#F9FAFB] hover:bg-[#F97316]/5 rounded-xl transition-colors group">
              <div className="w-8 h-8 shrink-0 rounded-lg bg-white border border-[#E5E7EB] flex items-center justify-center group-hover:border-[#F97316]/30 group-hover:shadow-sm transition-all">
                <ChefHat className="w-4 h-4" style={{color:'#F97316'}} />
              </div>
              <div>
                <p className="text-[11px] font-bold text-[#111827] group-hover:text-[#F97316] transition-colors">AI Recipe Generator</p>
                <p className="text-[9px] text-[#6B7280]">Generate delicious recipes</p>
              </div>
            </a>
            <a href="/tools/ai-workout-plan" className="flex items-center gap-3 p-2.5 bg-[#F9FAFB] hover:bg-[#EF4444]/5 rounded-xl transition-colors group">
              <div className="w-8 h-8 shrink-0 rounded-lg bg-white border border-[#E5E7EB] flex items-center justify-center group-hover:border-[#EF4444]/30 group-hover:shadow-sm transition-all">
                <Dumbbell className="w-4 h-4" style={{color:'#EF4444'}} />
              </div>
              <div>
                <p className="text-[11px] font-bold text-[#111827] group-hover:text-[#EF4444] transition-colors">AI Workout Planner</p>
                <p className="text-[9px] text-[#6B7280]">Create custom workout routines</p>
              </div>
            </a>
          </div>
        </div>
      ...
      {showHistory ? (
        ...
        <main className="flex-grow flex flex-col min-w-0">
          ...
          <div className="flex flex-col md:flex-row md:items-start lg:items-center justify-between gap-4 mb-6">
            <div>
              <h1 className="text-3xl lg:text-4xl font-bold tracking-tight text-[#111827] flex items-center gap-2">AI Meal Planner <Sparkles className="w-6 h-6" style={{ color: '#F97316' }} /></h1>
              <p className="text-[#6B7280] text-sm lg:text-base mt-2">
                Generate weekly meal plans with specific dietary constraints.
              </p>
            </div>
          ...
          <div className="flex-grow bg-white border border-[#E5E7EB] rounded-3xl p-6 lg:p-8 shadow-sm flex flex-col h-[600px] animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both delay-150">
            {isProcessing ? (
              <TextGenerationProgress title="Planning Your Meals..." description="Creating a balanced and healthy meal plan." />
            ) : result ? (
              <>
                ...
                <div className="flex flex-wrap items-center gap-3 pt-6 border-t border-[#E5E7EB] shrink-0">
                  ...
                  <button
                    onClick={async () => {
                      if (navigator.share) {
                        try {
                          await navigator.share({ title: 'A I  Meal Planner', text: result });
                        } catch (err) {
      ...
      <TextDownloadModal 
        ...
        content={result} 
        filename="A I  Meal Planner" 
        toolSlug="ai-meal-planner" 
