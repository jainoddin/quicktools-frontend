...
import { Download, History, Loader2, Copy, CheckCircle2, Sparkles, RefreshCw, Share2, Building2, Megaphone, Handshake } from 'lucide-react';
...
export default function ConflictResolverClient() {
  ...
  const localStorageKey = 'guest_ai-conflict-resolver_history';
  const toolSlug = '/tools/ai-conflict-resolver';
          ...
          if (data.success && data.data?.history) {
            const items = data.data.history
              ...
              .map((item: any) => ({
                ...
                prompt: item.prompt,
                contentType: 'AI Conflict Resolver',
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch(getEndpoint('/api/tools/ai-conflict-resolver'), {
        method: 'POST',
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-8 h-full">
      ...
      <aside className="w-full lg:w-[340px] shrink-0 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both">
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            <label className="flex items-center gap-2 text-sm font-bold text-[#111827]">
              Describe the conflict or paste the angry message
            </label>
        ...
        <div className="pt-2">
          <button
            ...
            style={{ backgroundColor: isProcessing || !inputVal.trim() ? '' : '#F59E0B' }}
          >
            {isProcessing ? (
              <><Loader2 className="w-5 h-5 animate-spin" /> Generating...</>
            ) : (
              <><Handshake className="w-5 h-5" /> Generate Results</>
            )}
          </button>
        </div>
      ...
      {showHistory ? (
        ...
        <main className="flex-grow flex flex-col min-w-0">
          ...
          <div className="flex flex-col md:flex-row md:items-start lg:items-center justify-between gap-4 mb-6">
            <div>
              <h1 className="text-3xl lg:text-4xl font-bold tracking-tight text-[#111827] flex items-center gap-2">AI Conflict Resolver <Sparkles className="w-6 h-6" style={{ color: '#F59E0B' }} /></h1>
              <p className="text-[#6B7280] text-sm lg:text-base mt-2">
                Craft calm, mature, and de-escalating text messages to resolve conflicts with partners or bosses.
              </p>
            </div>
                ...
                <div className="flex flex-wrap items-center gap-3 pt-6 border-t border-[#E5E7EB] shrink-0">
                  ...
                  <button
                    onClick={async () => {
                      if (navigator.share) {
                        try {
                          await navigator.share({ title: 'AI Conflict Resolver', text: result });
                        } catch (err) {
      ...
      <TextDownloadModal 
        ...
        content={result} 
        filename="AI Conflict Resolver" 
        toolSlug="ai-conflict-resolver" 
