...
import { Download, FileX, History, Loader2, Copy, CheckCircle2, Sparkles, Target, RefreshCw, Share2, FileText } from 'lucide-react';
...
export default function AiResignationLetterClient() {
  ...
  const localStorageKey = 'guestAiResignationLetterHistory';
  const toolSlug = '/tools/ai-resignation-letter';
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch(getEndpoint('/api/tools/resignation-letter'), {
        method: 'POST',
      ...
      if (data.success) {
        ...
        if (!isAuthenticated) {
          const historyItem = {
            ...
            contentType: 'Resignation Letter Generator',
    ...
    <div className="flex flex-col lg:flex-row gap-8 h-full">
      ...
      <aside className="w-full lg:w-[340px] shrink-0 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both">
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="e.g., Leaving my job as a Graphic Designer in 2 weeks to pursue a new opportunity"
            className="w-full h-40 p-4 bg-white rounded-2xl border border-[#E5E7EB] text-sm text-[#4B5563] placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#64748B]/20 focus:border-[#64748B] transition-all resize-none shadow-sm"
          />
        </div>
        ...
        <div className="pt-2">
          <button
            ...
            className={`w-full text-white font-bold py-3.5 rounded-xl transition-all shadow-md hover:shadow-lg flex items-center justify-center gap-2 ${isProcessing || !inputVal.trim() ? 'bg-[#9CA3AF] cursor-not-allowed' : 'bg-[#64748B] hover:opacity-90'}`}
          >
            {isProcessing ? (
              <><Loader2 className="w-5 h-5 animate-spin" /> Generating...</>
            ) : (
              <><FileX className="w-5 h-5" /> Generate Letter</>
            )}
          </button>
        </div>
        ...
        <div className="mt-4 pt-4 border-t border-[#E5E7EB]">
          ...
          <div className="flex flex-col gap-2">
            <a href="/tools/ai-apology-letter" className="flex items-center gap-3 p-2.5 bg-[#F9FAFB] hover:bg-[#6D5EF8]/5 rounded-xl transition-colors group">
              <div className="w-8 h-8 shrink-0 rounded-lg bg-white border border-[#E5E7EB] flex items-center justify-center group-hover:border-[#6D5EF8]/30 group-hover:shadow-sm transition-all">
                <FileText className="w-4 h-4" style={{color:'#6D5EF8'}} />
              </div>
              <div>
                <p className="text-[11px] font-bold text-[#111827] group-hover:text-[#6D5EF8] transition-colors">AI Apology Letter</p>
                <p className="text-[9px] text-[#6B7280]">Write sincere apology letters</p>
              </div>
            </a>
            <a href="/tools/ai-cover-letter" className="flex items-center gap-3 p-2.5 bg-[#F9FAFB] hover:bg-[#10B981]/5 rounded-xl transition-colors group">
              ...
              <div>
                <p className="text-[11px] font-bold text-[#111827] group-hover:text-[#10B981] transition-colors">AI Cover Letter</p>
                <p className="text-[9px] text-[#6B7280]">Create job-winning cover letters</p>
              </div>
            </a>
          </div>
        </div>
      ...
      {showHistory ? (
        <div className="flex-grow bg-white border border-[#E5E7EB] rounded-3xl p-6 shadow-sm overflow-y-auto h-[600px] animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both">
          <ToolHistorySidebar
            toolName="Resignation Letter Generator"
            toolType="text"
            history={toolHistory}
        ...
        <main className="flex-grow flex flex-col min-w-0">
          ...
          <div className="flex flex-col md:flex-row md:items-start lg:items-center justify-between gap-4 mb-6">
            <div>
              <h1 className="text-3xl lg:text-4xl font-bold tracking-tight text-[#111827] flex items-center gap-2">AI Resignation Letter Generator <Sparkles className="w-6 h-6" style={{ color: '#64748B' }} /></h1>
              <p className="text-[#6B7280] text-sm lg:text-base mt-2">
                Generates professional resignation letters tailored to your situation.
              </p>
            </div>
          ...
          {result && !isProcessing && (
            <div className="flex items-center justify-between mb-4 mt-2">
              <h2 className="text-xl font-extrabold text-[#111827] flex items-center gap-2">
                <Sparkles className="w-6 h-6" style={{ color: '#64748B' }} />
                Generated Result
              </h2>
          ...
          <div className="flex-grow bg-white border border-[#E5E7EB] rounded-3xl p-6 lg:p-8 shadow-sm flex flex-col h-[600px] animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both delay-150">
            {isProcessing ? (
              <TextGenerationProgress title="Writing Letter..." description="Crafting a professional resignation letter." />
            ) : result ? (
              <>
                ...
                <div className="flex flex-wrap items-center gap-3 pt-6 border-t border-[#E5E7EB] shrink-0">
                  ...
                  <button
                    onClick={async () => {
                      if (navigator.share) {
                        try {
                          await navigator.share({ title: 'A I  Resignation Letter', text: result });
                        } catch (err) {
      ...
      <TextDownloadModal 
        ...
        content={result} 
        filename="A I  Resignation Letter" 
        toolSlug="ai-resignation-letter" 
