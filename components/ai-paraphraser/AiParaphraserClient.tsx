...
import { 
  RefreshCw, History, Loader2, Copy, CheckCircle2,
  Sparkles, SpellCheck, PenTool
} from 'lucide-react';
...
export default function AiParaphraserClient() {
  ...
  useEffect(() => {
    if (!isAuthenticated) {
      const saved = localStorage.getItem('guestParaphraserHistory');
        ...
        .then(data => {
          if (data.success && data.data?.history) {
            const items = data.data.history
              .filter((item: any) => item.toolSlug === '/tools/ai-paraphraser')
              .map((item: any) => ({
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch(getEndpoint('/api/tools/paraphraser'), {
        method: 'POST',
      ...
      if (data.success) {
        ...
        if (!isAuthenticated) {
          ...
          localStorage.setItem('guestParaphraserHistory', JSON.stringify(newHistory));
        } else {
          setToolHistory(prev => [{
            id: data.usageId || Date.now().toString(),
            prompt: text.substring(0, 100),
      ...
      <aside className="w-full lg:w-[340px] shrink-0 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both">
        ...
        <div>
          ...
          <textarea
            ...
            className="w-full h-56 p-4 bg-white rounded-2xl border border-[#E5E7EB] text-sm text-[#4B5563] placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#10B981]/20 focus:border-[#10B981] transition-all resize-none shadow-sm"
          />
        </div>
        ...
        <div className="pt-2">
          <button
            ...
            className={`w-full text-white font-bold py-3.5 rounded-xl transition-all shadow-md hover:shadow-lg flex items-center justify-center gap-2 ${isProcessing || !text.trim() ? 'bg-[#9CA3AF] cursor-not-allowed' : 'bg-[#10B981] hover:bg-[#059669]'}`}
          >
            {isProcessing ? (
          ...
          <div className="flex flex-col gap-2">
            <a href="/tools/ai-grammar-checker" className="flex items-center gap-3 p-2.5 bg-[#F9FAFB] hover:bg-[#ECFDF5] rounded-xl transition-colors group">
              <div className="w-8 h-8 shrink-0 rounded-lg bg-white border border-[#E5E7EB] flex items-center justify-center group-hover:border-[#10B981]/30 group-hover:shadow-sm transition-all">
                <SpellCheck className="w-4 h-4 text-[#10B981]" />
              </div>
              <div>
                <p className="text-[11px] font-bold text-[#111827] group-hover:text-[#10B981] transition-colors">Grammar Checker</p>
                <p className="text-[9px] text-[#6B7280]">Fix spelling & phrasing</p>
              </div>
            </a>
            <a href="/tools/ai-writer" className="flex items-center gap-3 p-2.5 bg-[#F9FAFB] hover:bg-[#ECFDF5] rounded-xl transition-colors group">
              <div className="w-8 h-8 shrink-0 rounded-lg bg-white border border-[#E5E7EB] flex items-center justify-center group-hover:border-[#10B981]/30 group-hover:shadow-sm transition-all">
                <PenTool className="w-4 h-4 text-[#10B981]" />
              </div>
              <div>
                <p className="text-[11px] font-bold text-[#111827] group-hover:text-[#10B981] transition-colors">AI Writer</p>
                <p className="text-[9px] text-[#6B7280]">Write completely new text</p>
              </div>
            </a>
          </div>
        </div>
        ...
        <main className="flex-grow flex flex-col min-w-0">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-2xl font-bold text-[#111827] flex items-center gap-2">
                    AI Paraphrasing Tool <Sparkles className="w-5 h-5 text-[#10B981]" />
                  </h1>
                  <p className="text-sm text-[#6B7280]">Rewrite text perfectly while retaining its original meaning.</p>
                </div>
              </div>
          ...
          {isProcessing ? (
            <div className="flex-grow bg-white rounded-3xl border border-[#E5E7EB] p-8 flex flex-col items-center justify-center shadow-sm animate-in fade-in duration-500 h-[600px]">
              ...
              <h2 className="text-xl font-bold text-[#111827]">Rewriting Text...</h2>
              <p className="text-[#6B7280] mt-2">Finding the perfect synonyms and sentence structures.</p>
            </div>
          ) : result ? (
            <div className="flex-grow flex flex-col bg-white border border-[#E5E7EB] rounded-3xl p-6 shadow-sm animate-in zoom-in-95 duration-500 h-[600px]">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-bold text-[#111827]">Paraphrased Result</h2>
              ...
              <div className="flex-1 bg-[#FAFAFA] p-6 rounded-2xl border border-[#E5E7EB] overflow-y-auto">
                <p className="text-sm text-[#374151] whitespace-pre-wrap leading-relaxed">{result}</p>
              </div>
            </div>
          ) : (
            <div className="flex-grow bg-white rounded-3xl border border-[#E5E7EB] p-8 lg:p-12 shadow-sm flex flex-col items-center justify-center text-center relative animate-in fade-in slide-in-from-bottom-8 duration-700 fill-mode-both delay-200 h-[600px]">
              ...
              <div className="relative w-72 h-64 mb-10 flex items-center justify-center mt-4">
                ...
                <div className="relative z-10 w-40 h-40 bg-white rounded-2xl shadow-[0_10px_30px_-10px_rgba(16,185,129,0.3)] border-4 border-emerald-200 overflow-hidden flex items-center justify-center">
                  <RefreshCw className="w-20 h-20 text-[#10B981] animate-[spin_4s_linear_infinite]" />
                </div>
                
                <div className="absolute top-8 left-12 w-6 h-6 text-emerald-300 animate-pulse">✨</div>
                <div className="absolute bottom-8 right-12 w-5 h-5 text-emerald-300 animate-pulse" style={{ animationDelay: '500ms' }}>✨</div>
              </div>

              <h2 className="text-2xl lg:text-3xl font-bold text-[#111827] mb-3">
                Need to <span className="text-[#10B981]">rephrase?</span>
              </h2>
              <p className="text-[#6B7280] max-w-sm mx-auto mb-8">
                Paste your text on the left and click <span className="text-[#10B981] font-semibold">"Paraphrase Text"</span> to rewrite it uniquely.
              </p>
            </div>
          )}
        </main>
