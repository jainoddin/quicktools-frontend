...
import { CheckCircle2, Loader2, Circle, FileText, Smile, Sparkles } from 'lucide-react';

interface AiWriterProgressProps {
  progress: number;
  onCancel: () => void;
}

export default function AiWriterProgress({ progress, onCancel }: AiWriterProgressProps) {
  let currentStep = 0;
  ...
  return (
    <div className="flex-grow min-h-[500px] bg-white rounded-3xl border border-[#E5E7EB] flex flex-col items-center p-8 shadow-sm relative overflow-hidden animate-in fade-in duration-300">
      
      <div className="text-center mb-8 mt-4">
        <h2 className="text-2xl font-bold text-[#111827] mb-2">Writing your content...</h2>
      ...
      <div className="bg-[#F8FAFC] text-[#6B7280] text-xs font-semibold px-4 py-2 rounded-lg border border-[#E5E7EB] mb-12">
        Estimated time: {Math.max(1, Math.ceil((100 - progress) / 25))} seconds
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-8 w-full max-w-2xl mb-8 items-start">
        ...
        <div className="flex flex-col gap-6 relative">
          ...
          <StepItem active={currentStep >= 1} completed={currentStep > 1} text="Analyzing your prompt" />
          <StepItem active={currentStep >= 2} completed={currentStep > 2} text="Gathering relevant ideas" />
          <StepItem active={currentStep >= 3} completed={currentStep > 3} text="Drafting the content" />
          <StepItem active={currentStep >= 4} completed={currentStep > 4} text="Refining tone and style" />
        </div>

        {/* Right: Tips */}
        <div className="flex flex-col gap-4">
          <TipCard icon={FileText} text="Specific prompts give you much better content." />
          <TipCard icon={Smile} text="Experiment with different tones to match your audience." />
          <TipCard icon={Sparkles} text="Our AI uses the latest models for human-like writing." />
        </div>
      </div>
