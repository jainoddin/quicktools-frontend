...
import { Download, BookOpen, History, Loader2, Copy, CheckCircle2, Sparkles, Target, RefreshCw, Share2, PenLine, Lightbulb } from 'lucide-react';
...
export default function AiStoryGeneratorClient() {
  ...
  const localStorageKey = 'guestAiStoryGeneratorHistory';
  const toolSlug = '/tools/ai-story-generator';
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch(getEndpoint('/api/tools/story-generator'), {
        method: 'POST',
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-8 h-full">
      ...
      <aside className="w-full lg:w-[340px] shrink-0 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both">
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="e.g., A sci-fi story about a detective who can read memories"
            className="w-full h-40 p-4 bg-white rounded-2xl border border-[#E5E7EB] text-sm text-[#4B5563] placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#6D5EF8]/20 focus:border-[#6D5EF8] transition-all resize-none shadow-sm"
          />
        </div>
        ...
        <div className="pt-2">
          <button
            ...
            className={`w-full text-white font-bold py-3.5 rounded-xl transition-all shadow-md hover:shadow-lg flex items-center justify-center gap-2 ${isProcessing || !inputVal.trim() ? 'bg-[#9CA3AF] cursor-not-allowed' : 'bg-[#6D5EF8] hover:opacity-90'}`}
          >
            {isProcessing ? (
          ...
          <div className="flex flex-col gap-2">
            <a href="/tools/ai-poem-generator" className="flex items-center gap-3 p-2.5 bg-[#F9FAFB] hover:bg-[#8B5CF6]/5 rounded-xl transition-colors group">
              <div className="w-8 h-8 shrink-0 rounded-lg bg-white border border-[#E5E7EB] flex items-center justify-center group-hover:border-[#8B5CF6]/30 group-hover:shadow-sm transition-all">
                <PenLine className="w-4 h-4" style={{color:'#8B5CF6'}} />
              </div>
              <div>
                <p className="text-[11px] font-bold text-[#111827] group-hover:text-[#8B5CF6] transition-colors">AI Poem Generator</p>
                <p className="text-[9px] text-[#6B7280]">Write beautiful poems instantly</p>
              </div>
            </a>
            <a href="/tools/ai-blog-idea-generator" className="flex items-center gap-3 p-2.5 bg-[#F9FAFB] hover:bg-[#F59E0B]/5 rounded-xl transition-colors group">
              ...
              <div>
                <p className="text-[11px] font-bold text-[#111827] group-hover:text-[#F59E0B] transition-colors">AI Blog Idea Generator</p>
                <p className="text-[9px] text-[#6B7280]">Get creative blog topic ideas</p>
              </div>
            </a>
          </div>
        </div>
      ...
      {showHistory ? (
        ...
        <main className="flex-grow flex flex-col min-w-0">
          ...
          <div className="flex flex-col md:flex-row md:items-start lg:items-center justify-between gap-4 mb-6">
            <div>
              <h1 className="text-3xl lg:text-4xl font-bold tracking-tight text-[#111827] flex items-center gap-2">AI Story Generator <Sparkles className="w-6 h-6" style={{ color: '#6D5EF8' }} /></h1>
              <p className="text-[#6B7280] text-sm lg:text-base mt-2">
                Write creative short stories based on a prompt and genre.
              </p>
            </div>
          ...
          {result && !isProcessing && (
            <div className="flex items-center justify-between mb-4 mt-2">
              <h2 className="text-xl font-extrabold text-[#111827] flex items-center gap-2">
                <Sparkles className="w-6 h-6" style={{ color: '#6D5EF8' }} />
                Generated Result
              </h2>
          ...
          <div className="flex-grow bg-white border border-[#E5E7EB] rounded-3xl p-6 lg:p-8 shadow-sm flex flex-col h-[600px] animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both delay-150">
            {isProcessing ? (
              <TextGenerationProgress title="Writing Your Story..." description="Crafting a creative narrative just for you." />
            ) : result ? (
              <>
                ...
                <div className="flex flex-wrap items-center gap-3 pt-6 border-t border-[#E5E7EB] shrink-0">
                  ...
                  <button
                    onClick={async () => {
                      if (navigator.share) {
                        try {
                          await navigator.share({ title: 'A I  Story Generator', text: result });
                        } catch (err) {
      ...
      <TextDownloadModal 
        ...
        content={result} 
        filename="A I  Story Generator" 
        toolSlug="ai-story-generator" 
