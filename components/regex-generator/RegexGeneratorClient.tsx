...
import { Code2, History, Loader2, Copy, CheckCircle2, Sparkles, Code, Share2, RefreshCw, Download } from 'lucide-react';
...
export default function RegexGeneratorClient() {
  ...
  const handleToggleFavorite = async (id: string) => {
    setToolHistory(prev => {
      ...
      if (!isAuthenticated) {
        localStorage.setItem('guestRegexGeneratorHistory', JSON.stringify(updated));
      }
      return updated;
    });
    if (isAuthenticated) {
      ...
      {showHistory ? (
        ...
        <main className="flex-grow flex flex-col min-w-0">
          ...
          <div className="flex flex-col md:flex-row md:items-start lg:items-center justify-between gap-4 mb-6">
            <div>
              <h1 className="text-3xl lg:text-4xl font-bold tracking-tight text-[#111827] flex items-center gap-2">
                AI Regex Generator <Sparkles className="w-6 h-6" style={{ color: 'bg-[#3B82F6] text-white' }} />
              </h1>
          ...
          <div className="flex-grow bg-white border border-[#E5E7EB] rounded-3xl p-6 lg:p-8 shadow-sm flex flex-col h-[600px] animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both delay-150">
            {isProcessing ? (
              <TextGenerationProgress title="Translating to Regex..." description="Writing optimal patterns and matching groups." />
            ) : result ? (
              <>
