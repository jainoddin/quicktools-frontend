...
import { 
  Code2, History, Loader2, Copy, CheckCircle2,
  Sparkles, Code
} from 'lucide-react';
...
export default function RegexGeneratorClient() {
  ...
  useEffect(() => {
    if (!isAuthenticated) {
      const saved = localStorage.getItem('guestRegexGeneratorHistory');
        ...
        .then(data => {
          if (data.success && data.data?.history) {
            const items = data.data.history
              .filter((item: any) => item.toolSlug === '/tools/regex-generator')
              .map((item: any) => ({
  ...
  const handleGenerate = async () => {
    ...
    try {
      // Note: We need a backend route for this. Wait, did we add it?
      // Since it wasn't listed in the 17, I'll need to hit a generic generation endpoint or one specifically for regex.
      // Wait, there's no generic text endpoint? Let's use a dynamic one or assume we created one.
      // Actually, since I didn't create a route for regex in tools.routes.ts, I will create one now or use a generic approach.
      // Let's assume we have an endpoint, or I'll quickly add one in the backend if it's missing later.
      // But for now, let's use a fallback to /api/tools/article-outline or something? No, that won't work.
      // I'll add the endpoint in the next step. Let's just write the client assuming `/api/tools/regex-generator` exists.
      const res = await fetch(getEndpoint('/api/tools/regex-generator'), {
        method: 'POST',
      ...
      if (data.success) {
        ...
        if (!isAuthenticated) {
          ...
          localStorage.setItem('guestRegexGeneratorHistory', JSON.stringify(newHistory));
        } else {
          setToolHistory(prev => [{
            id: data.usageId || Date.now().toString(),
            prompt: description.substring(0, 100),
      ...
      <aside className="w-full lg:w-[340px] shrink-0 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both">
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-xs text-[#6B7280]">{description.length}/500</span>
          </div>
          <textarea
            value={description}
            onChange={(e) => setDescription(e.target.value.substring(0, 500))}
            placeholder="e.g., Match a valid email address ending in @gmail.com..."
            className="w-full h-40 p-4 bg-white rounded-2xl border border-[#E5E7EB] text-sm text-[#4B5563] placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#0EA5E9]/20 focus:border-[#0EA5E9] transition-all resize-none shadow-sm font-mono"
          />
        </div>
        ...
        <div className="pt-2">
          <button
            ...
            className={`w-full text-white font-bold py-3.5 rounded-xl transition-all shadow-md hover:shadow-lg flex items-center justify-center gap-2 ${isProcessing || !description.trim() ? 'bg-[#9CA3AF] cursor-not-allowed' : 'bg-[#0EA5E9] hover:bg-[#0284C7]'}`}
          >
            {isProcessing ? (
              <><Loader2 className="w-5 h-5 animate-spin" /> Generating...</>
            ) : (
              <><Code2 className="w-5 h-5" /> Generate Regex</>
            )}
          </button>
        </div>
        ...
        <div className="mt-4 pt-4 border-t border-[#E5E7EB]">
          ...
          <div className="flex flex-col gap-2">
            <a href="/tools/json-formatter" className="flex items-center gap-3 p-2.5 bg-[#F9FAFB] hover:bg-[#F0F9FF] rounded-xl transition-colors group">
              ...
              <div>
                <p className="text-[11px] font-bold text-[#111827] group-hover:text-[#0EA5E9] transition-colors">JSON Formatter</p>
                <p className="text-[9px] text-[#6B7280]">Format & validate JSON</p>
              </div>
            </a>
          </div>
        </div>
        ...
        <main className="flex-grow flex flex-col min-w-0">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-2xl font-bold text-[#111827] flex items-center gap-2">
                    AI Regex Generator <Sparkles className="w-5 h-5 text-[#0EA5E9]" />
                  </h1>
                  <p className="text-sm text-[#6B7280]">Generate complex Regular Expressions easily from plain English descriptions.</p>
                </div>
              </div>
          ...
          {isProcessing ? (
            <div className="flex-grow bg-white rounded-3xl border border-[#E5E7EB] p-8 flex flex-col items-center justify-center shadow-sm animate-in fade-in duration-500 h-[600px]">
              ...
              <h2 className="text-xl font-bold text-[#111827]">Translating to Regex...</h2>
              <p className="text-[#6B7280] mt-2">Writing optimal patterns and matching groups.</p>
            </div>
          ) : result ? (
            <div className="flex-grow flex flex-col bg-white border border-[#E5E7EB] rounded-3xl p-6 shadow-sm animate-in zoom-in-95 duration-500 h-[600px]">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-bold text-[#111827]">Generated Regular Expression</h2>
              ...
              <div className="flex-1 bg-[#1E293B] p-8 rounded-2xl border border-[#0F172A] overflow-y-auto">
            ...
            <div className="flex-grow bg-white rounded-3xl border border-[#E5E7EB] p-8 lg:p-12 shadow-sm flex flex-col items-center justify-center text-center relative animate-in fade-in slide-in-from-bottom-8 duration-700 fill-mode-both delay-200 h-[600px]">
              ...
              <div className="relative w-72 h-64 mb-10 flex items-center justify-center mt-4">
                ...
                <div className="relative z-10 w-44 h-32 bg-white rounded-xl shadow-[0_10px_30px_-10px_rgba(14,165,233,0.3)] border-4 border-sky-200 overflow-hidden flex flex-col items-center justify-center transform hover:scale-110 transition-transform duration-500">
                  <Code2 className="w-16 h-16 text-[#0EA5E9] mb-2 drop-shadow-md animate-[pulse_2s_infinite]" />
                </div>
                
                <div className="absolute top-4 -right-4 w-12 h-12 bg-[#1E293B] rounded-full shadow-lg border-2 border-sky-200 flex items-center justify-center animate-[bounce_3s_infinite]">
                  <span className="font-bold text-sky-400 text-xs">/.*/</span>
                </div>
              </div>

              <h2 className="text-2xl lg:text-3xl font-bold text-[#111827] mb-3">
                Need <span className="text-[#0EA5E9]">RegEx?</span>
              </h2>
              <p className="text-[#6B7280] max-w-sm mx-auto mb-8">
                Describe the pattern you want to match on the left and click <span className="text-[#0EA5E9] font-semibold">"Generate Regex"</span>.
              </p>
            </div>
          )}
        </main>
