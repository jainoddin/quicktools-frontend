...
import { 
  Hash, History, Loader2, Copy, CheckCircle2,
  Sparkles, Image as ImageIcon, Video, Heart } from 'lucide-react';
...
export default function AiCaptionClient() {
  ...
  useEffect(() => {
    if (!isAuthenticated) {
      const saved = localStorage.getItem('guestCaptionHistory');
        ...
        .then(data => {
          if (data.success && data.data?.history) {
            const items = data.data.history
              .filter((item: any) => item.toolSlug === '/tools/ai-caption-generator')
              .map((item: any) => ({
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch(getEndpoint('/api/tools/caption'), {
        method: 'POST',
      ...
      if (data.success) {
        ...
        if (!isAuthenticated) {
          ...
          localStorage.setItem('guestCaptionHistory', JSON.stringify(newHistory));
        } else {
          setToolHistory(prev => [{
            id: data.usageId || Date.now().toString(),
            prompt: topic.substring(0, 100),
      ...
      <aside className="w-full lg:w-[340px] shrink-0 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both">
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="e.g., A photo from my recent trip to the mountains of Manali..."
            className="w-full h-40 p-4 bg-white rounded-2xl border border-[#E5E7EB] text-sm text-[#4B5563] placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#8B5CF6]/20 focus:border-[#8B5CF6] transition-all resize-none shadow-sm"
          />
        </div>
        ...
        <div className="pt-2">
          <button
            ...
            className={`w-full text-white font-bold py-3.5 rounded-xl transition-all shadow-md hover:shadow-lg flex items-center justify-center gap-2 ${isProcessing || !topic.trim() ? 'bg-[#9CA3AF] cursor-not-allowed' : 'bg-[#8B5CF6] hover:bg-[#7C3AED]'}`}
          >
            {isProcessing ? (
              <><Loader2 className="w-5 h-5 animate-spin" /> Generating...</>
            ) : (
              <><Hash className="w-5 h-5" /> Generate Captions</>
            )}
          </button>
        </div>
        ...
        <div className="mt-4 pt-4 border-t border-[#E5E7EB]">
          ...
          <div className="flex flex-col gap-2">
            <a href="/tools/ai-image-generator" className="flex items-center gap-3 p-2.5 bg-[#F9FAFB] hover:bg-[#F5F3FF] rounded-xl transition-colors group">
              ...
              <div>
                <p className="text-[11px] font-bold text-[#111827] group-hover:text-[#8B5CF6] transition-colors">AI Image Generator</p>
                <p className="text-[9px] text-[#6B7280]">Generate stunning images</p>
              </div>
            </a>
            <a href="/tools/ai-video-generator" className="flex items-center gap-3 p-2.5 bg-[#F9FAFB] hover:bg-[#F5F3FF] rounded-xl transition-colors group">
              <div className="w-8 h-8 shrink-0 rounded-lg bg-white border border-[#E5E7EB] flex items-center justify-center group-hover:border-[#8B5CF6]/30 group-hover:shadow-sm transition-all">
                <Video className="w-4 h-4 text-[#8B5CF6]" />
              </div>
              <div>
                <p className="text-[11px] font-bold text-[#111827] group-hover:text-[#8B5CF6] transition-colors">AI Video Generator</p>
                <p className="text-[9px] text-[#6B7280]">Create videos from text</p>
              </div>
            </a>
          </div>
        </div>
        ...
        <main className="flex-grow flex flex-col min-w-0">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-2xl font-bold text-[#111827] flex items-center gap-2">
                    AI Caption Generator <Sparkles className="w-5 h-5 text-[#8B5CF6]" />
                  </h1>
                  <p className="text-sm text-[#6B7280]">Generate engaging captions with emojis and hashtags.</p>
                </div>
              </div>
          ...
          {isProcessing ? (
            <div className="flex-grow bg-white rounded-3xl border border-[#E5E7EB] p-8 flex flex-col items-center justify-center shadow-sm animate-in fade-in duration-500 h-[600px]">
              ...
              <h2 className="text-xl font-bold text-[#111827]">Brainstorming Captions...</h2>
              <p className="text-[#6B7280] mt-2">Adding emojis and finding trending hashtags.</p>
            </div>
          ) : result ? (
            <div className="flex-grow flex flex-col bg-white border border-[#E5E7EB] rounded-3xl p-6 shadow-sm animate-in zoom-in-95 duration-500 h-[600px]">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-bold text-[#111827]">Caption Ideas</h2>
            ...
            <div className="flex-grow bg-white rounded-3xl border border-[#E5E7EB] p-8 lg:p-12 shadow-sm flex flex-col items-center justify-center text-center relative animate-in fade-in slide-in-from-bottom-8 duration-700 fill-mode-both delay-200 h-[600px]">
              ...
              <div className="relative w-72 h-64 mb-10 flex items-center justify-center mt-4">
                ...
                <div className="relative z-10 w-40 h-40 bg-white rounded-2xl shadow-[0_10px_30px_-10px_rgba(139,92,246,0.3)] border-4 border-purple-200 overflow-hidden flex flex-col items-center justify-center transform hover:scale-105 transition-transform duration-500 animate-[bounce_4s_infinite]">
                  <Hash className="w-16 h-16 text-[#8B5CF6] mb-2" />
                  <div className="w-20 h-2 bg-purple-200 rounded-full"></div>
                </div>

                <div className="absolute top-6 left-6 w-12 h-12 bg-white rounded-full shadow-lg border border-[#E5E7EB] flex items-center justify-center animate-[bounce_3s_infinite_0.5s]">
                  <Heart className="w-6 h-6 text-red-500 fill-red-500" />
                </div>
                
                <div className="absolute bottom-10 right-6 w-12 h-12 bg-white rounded-full shadow-lg border border-[#E5E7EB] flex items-center justify-center animate-[bounce_3s_infinite_1s] text-xl">
                  🔥
                </div>

                <Sparkles className="absolute top-12 right-12 w-6 h-6 text-purple-300 animate-pulse" />
                <Sparkles className="absolute bottom-8 left-12 w-5 h-5 text-purple-300 animate-pulse" style={{ animationDelay: '500ms' }} />
              </div>

              <h2 className="text-2xl lg:text-3xl font-bold text-[#111827] mb-3">
                Ready to <span className="text-[#8B5CF6]">post?</span>
              </h2>
              <p className="text-[#6B7280] max-w-sm mx-auto mb-8">
                Describe your photo or topic on the left and click <span className="text-[#8B5CF6] font-semibold">"Generate Captions"</span>.
              </p>
            </div>
          )}
        </main>
