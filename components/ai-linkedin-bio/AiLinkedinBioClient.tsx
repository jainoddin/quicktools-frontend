...
import { Download, Share2, RefreshCw, Briefcase, History, Loader2, Copy, CheckCircle2, Sparkles } from 'lucide-react';
...
export default function AiLinkedinBioClient() {
  ...
  const handleToggleFavorite = async (id: string) => {
    setToolHistory(prev => {
      ...
      if (!isAuthenticated) {
        localStorage.setItem('guestBriefcaseBioHistory', JSON.stringify(updated));
      }
      return updated;
    });
    if (isAuthenticated) {
      ...
      {showHistory ? (
        ...
        <main className="flex-grow flex flex-col min-w-0">
          ...
          <div className="flex flex-col md:flex-row md:items-start lg:items-center justify-between gap-4 mb-6">
            <div>
              <h1 className="text-3xl lg:text-4xl font-bold tracking-tight text-[#111827] flex items-center gap-2">AI LinkedIn Bio <Sparkles className="w-6 h-6" style={{ color: '#0A66C2' }} /></h1>
              <p className="text-[#6B7280] text-sm lg:text-base mt-2">
                Write an optimized, professional LinkedIn About section.
              </p>
            </div>
          ...
          {result && !isProcessing && (
            <div className="flex items-center justify-between mb-4 mt-2">
              <h2 className="text-xl font-extrabold text-[#111827] flex items-center gap-2">
                <Sparkles className="w-6 h-6" style={{ color: '#0A66C2' }} />
                Generated Result
              </h2>
          ...
          <div className="flex-grow bg-white border border-[#E5E7EB] rounded-3xl p-6 lg:p-8 shadow-sm flex flex-col h-[600px] animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both delay-150">
            {isProcessing ? (
              <TextGenerationProgress title="Optimizing Profile..." description="Structuring your achievements for maximum impact." />
            ) : result ? (
              <>
