...
import { Download, Share2, RefreshCw, Briefcase, History, Loader2, Copy, CheckCircle2, Sparkles, Palette, Hash, TrendingUp } from 'lucide-react';
...
export default function AiBusinessNameClient() {
  ...
  const handleToggleFavorite = async (id: string) => {
    setToolHistory(prev => {
      ...
      if (!isAuthenticated) {
        localStorage.setItem('guestBusinessNameHistory', JSON.stringify(updated));
      }
      return updated;
    });
    if (isAuthenticated) {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-8 h-full">
      ...
      {showHistory ? (
        ...
        <main className="flex-grow flex flex-col min-w-0">
          ...
          <div className="flex flex-col md:flex-row md:items-start lg:items-center justify-between gap-4 mb-6">
            <div>
              <h1 className="text-3xl lg:text-4xl font-bold tracking-tight text-[#111827] flex items-center gap-2">AI Business Name Generator <Sparkles className="w-6 h-6" style={{ color: '#F59E0B' }} /></h1>
              <p className="text-[#6B7280] text-sm lg:text-base mt-2">
                Discover catchy, memorable names and taglines for your next startup.
              </p>
            </div>
          ...
          <div className="flex-grow bg-white border border-[#E5E7EB] rounded-3xl p-6 lg:p-8 shadow-sm flex flex-col h-[600px] animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both delay-150">
            {isProcessing ? (
              <TextGenerationProgress title="Brainstorming Names..." description="Thinking outside the box." />
            ) : result ? (
              <>
      ...
      <TextDownloadModal 
        ...
        toolSlug="ai-business-name-generator" 
