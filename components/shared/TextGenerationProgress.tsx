...
import { CheckCircle2, Loader2, Circle, FileText, CheckCircle, Sparkles } from 'lucide-react';
...
export default function TextGenerationProgress({ title, description }: TextGenerationProgressProps) {
  ...
  return (
    <div className="flex-grow min-h-[500px] h-full bg-white rounded-3xl flex flex-col items-center justify-center p-8 relative overflow-hidden">
      ...
      <div className="grid md:grid-cols-2 gap-8 w-full max-w-2xl items-start">
        
        {/* Left: Timeline */}
        <div className="flex flex-col gap-6 relative">
          <div className="absolute left-[11px] top-4 bottom-4 w-0.5 bg-gray-100 -z-10"></div>
          
          <StepItem active={currentStep >= 1} completed={currentStep > 1} text="Analyzing your request" />
          <StepItem active={currentStep >= 2} completed={currentStep > 2} text="Gathering context" />
          <StepItem active={currentStep >= 3} completed={currentStep > 3} text="Generating content" />
          <StepItem active={currentStep >= 4} completed={currentStep > 4} text="Finalizing and formatting" />
        </div>

        {/* Right: Tips */}
        <div className="flex flex-col gap-4">
          <TipCard icon={FileText} text="Be specific in your inputs for better results." />
          <TipCard icon={CheckCircle} text="You can always edit the generated text." />
          <TipCard icon={Sparkles} text="High quality generations take a little more time." />
        </div>
      </div>
