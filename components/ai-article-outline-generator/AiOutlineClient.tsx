...
import { 
  List, History, Loader2, Copy, CheckCircle2,
  Sparkles, PenTool, LayoutGrid, FileText } from 'lucide-react';
...
export default function AiOutlineClient() {
  ...
  useEffect(() => {
    if (!isAuthenticated) {
      const saved = localStorage.getItem('guestOutlineHistory');
        ...
        .then(data => {
          if (data.success && data.data?.history) {
            const items = data.data.history
              .filter((item: any) => item.toolSlug === '/tools/ai-article-outline-generator')
              .map((item: any) => ({
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch(getEndpoint('/api/tools/article-outline'), {
        method: 'POST',
      ...
      if (data.success) {
        ...
        if (!isAuthenticated) {
          ...
          localStorage.setItem('guestOutlineHistory', JSON.stringify(newHistory));
        } else {
          setToolHistory(prev => [{
            id: data.usageId || Date.now().toString(),
            prompt: topic.substring(0, 100),
      ...
      <aside className="w-full lg:w-[340px] shrink-0 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both">
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="e.g., The Future of Electric Vehicles in 2030..."
            className="w-full h-40 p-4 bg-white rounded-2xl border border-[#E5E7EB] text-sm text-[#4B5563] placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#3B82F6]/20 focus:border-[#3B82F6] transition-all resize-none shadow-sm"
          />
        </div>
        ...
        <div className="pt-2">
          <button
            ...
            className={`w-full text-white font-bold py-3.5 rounded-xl transition-all shadow-md hover:shadow-lg flex items-center justify-center gap-2 ${isProcessing || !topic.trim() ? 'bg-[#9CA3AF] cursor-not-allowed' : 'bg-[#3B82F6] hover:bg-[#2563EB]'}`}
          >
            {isProcessing ? (
              <><Loader2 className="w-5 h-5 animate-spin" /> Structuring...</>
            ) : (
              <><List className="w-5 h-5" /> Generate Outline</>
            )}
          </button>
        </div>
        ...
        <div className="mt-4 pt-4 border-t border-[#E5E7EB]">
          ...
          <div className="flex flex-col gap-2">
            <a href="/tools/ai-writer" className="flex items-center gap-3 p-2.5 bg-[#F9FAFB] hover:bg-[#EFF6FF] rounded-xl transition-colors group">
              ...
              <div>
                <p className="text-[11px] font-bold text-[#111827] group-hover:text-[#3B82F6] transition-colors">AI Writer</p>
                <p className="text-[9px] text-[#6B7280]">Write the full article</p>
              </div>
            </a>
            <a href="/tools/ai-blog-idea-generator" className="flex items-center gap-3 p-2.5 bg-[#F9FAFB] hover:bg-[#EFF6FF] rounded-xl transition-colors group">
              <div className="w-8 h-8 shrink-0 rounded-lg bg-white border border-[#E5E7EB] flex items-center justify-center group-hover:border-[#3B82F6]/30 group-hover:shadow-sm transition-all">
                <FileText className="w-4 h-4 text-[#3B82F6]" />
              </div>
              <div>
                <p className="text-[11px] font-bold text-[#111827] group-hover:text-[#3B82F6] transition-colors">Blog Idea Generator</p>
                <p className="text-[9px] text-[#6B7280]">Find new topics to write</p>
              </div>
            </a>
          </div>
        </div>
        ...
        <main className="flex-grow flex flex-col min-w-0">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-2xl font-bold text-[#111827] flex items-center gap-2">
                    AI Article Outline <Sparkles className="w-5 h-5 text-[#3B82F6]" />
                  </h1>
                  <p className="text-sm text-[#6B7280]">Generate comprehensive, SEO-optimized article outlines instantly.</p>
                </div>
              </div>
          ...
          {isProcessing ? (
            <div className="flex-grow bg-white rounded-3xl border border-[#E5E7EB] p-8 flex flex-col items-center justify-center shadow-sm animate-in fade-in duration-500 h-[600px]">
              ...
              <h2 className="text-xl font-bold text-[#111827]">Structuring Outline...</h2>
              <p className="text-[#6B7280] mt-2">Organizing H1, H2, and H3 headers.</p>
            </div>
          ) : result ? (
            <div className="flex-grow flex flex-col bg-white border border-[#E5E7EB] rounded-3xl p-6 shadow-sm animate-in zoom-in-95 duration-500 h-[600px]">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-bold text-[#111827]">Generated Outline</h2>
            ...
            <div className="flex-grow bg-white rounded-3xl border border-[#E5E7EB] p-8 lg:p-12 shadow-sm flex flex-col items-center justify-center text-center relative animate-in fade-in slide-in-from-bottom-8 duration-700 fill-mode-both delay-200 h-[600px]">
              ...
              <div className="relative w-72 h-64 mb-10 flex items-center justify-center mt-4">
                ...
                <div className="relative z-10 w-40 h-44 bg-white rounded-2xl shadow-[0_10px_30px_-10px_rgba(59,130,246,0.3)] border-4 border-blue-200 overflow-hidden flex flex-col p-4 animate-[bounce_4s_infinite]">
                  <div className="w-full h-3 bg-blue-400 rounded-full mb-4"></div>
                ...
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-52 h-52 border-2 border-dashed border-blue-200 rounded-full animate-[spin_12s_linear_infinite]"></div>

                <Sparkles className="absolute top-8 left-12 w-6 h-6 text-blue-300 animate-pulse" />
                <Sparkles className="absolute bottom-8 right-12 w-5 h-5 text-blue-300 animate-pulse" style={{ animationDelay: '500ms' }} />
              </div>

              <h2 className="text-2xl lg:text-3xl font-bold text-[#111827] mb-3">
                Need an <span className="text-[#3B82F6]">outline?</span>
              </h2>
              <p className="text-[#6B7280] max-w-sm mx-auto mb-8">
                Enter your article topic on the left and click <span className="text-[#3B82F6] font-semibold">"Generate Outline"</span>.
              </p>
            </div>
          )}
        </main>
