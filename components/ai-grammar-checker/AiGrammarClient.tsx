...
import { SpellCheck, History, Loader2, Copy, CheckCircle2, Sparkles, PenTool, LayoutGrid, Wand2, Image as ImageIcon, Share2, RefreshCw, Download } from 'lucide-react';
...
export default function AiGrammarClient() {
  ...
  const handleToggleFavorite = async (id: string) => {
    setToolHistory(prev => {
      ...
      if (!isAuthenticated) {
        localStorage.setItem('guestGrammarHistory', JSON.stringify(updated));
      }
      return updated;
    });
    if (isAuthenticated) {
      ...
      {showHistory ? (
        ...
        <main className="flex-grow flex flex-col min-w-0">
          ...
          <div className="flex flex-col md:flex-row md:items-start lg:items-center justify-between gap-4 mb-6">
            <div>
              <h1 className="text-3xl lg:text-4xl font-bold tracking-tight text-[#111827] flex items-center gap-2">AI Grammar Checker <Sparkles className="w-6 h-6" style={{ color: '#EF4444' }} /></h1>
              <p className="text-[#6B7280] text-sm lg:text-base mt-2">
                Fix grammar, spelling, and phrasing instantly with AI.
              </p>
            </div>
          ...
          <div className="flex-grow bg-white border border-[#E5E7EB] rounded-3xl p-6 lg:p-8 shadow-sm flex flex-col h-[600px] animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both delay-150">
            {isProcessing ? (
              <TextGenerationProgress title="Checking Text..." description="Fixing typos and improving flow." />
            ) : result ? (
              <>
