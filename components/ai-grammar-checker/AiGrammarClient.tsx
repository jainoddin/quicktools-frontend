...
import { 
  SpellCheck, History, Loader2, Copy, CheckCircle2,
  Sparkles, PenTool, LayoutGrid, Wand2, Image as ImageIcon
} from 'lucide-react';
...
export default function AiGrammarClient() {
  ...
  useEffect(() => {
    if (!isAuthenticated) {
      const saved = localStorage.getItem('guestGrammarHistory');
        ...
        .then(data => {
          if (data.success && data.data?.history) {
            const items = data.data.history
              .filter((item: any) => item.toolSlug === '/tools/ai-grammar-checker')
              .map((item: any) => ({
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch(getEndpoint('/api/tools/grammar'), {
        method: 'POST',
      ...
      if (data.success) {
        ...
        if (!isAuthenticated) {
          ...
          localStorage.setItem('guestGrammarHistory', JSON.stringify(newHistory));
        } else {
          setToolHistory(prev => [{
            id: data.usageId || Date.now().toString(),
            prompt: text.substring(0, 100) + '...',
      ...
      <aside className="w-full lg:w-[340px] shrink-0 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both">
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-xs text-[#6B7280]">{text.length}/2000</span>
          </div>
          <textarea
            value={text}
            onChange={(e) => setText(e.target.value.substring(0, 2000))}
            placeholder="Paste your text here to fix grammar, spelling, and phrasing..."
            className="w-full h-48 p-4 bg-white rounded-2xl border border-[#E5E7EB] text-sm text-[#4B5563] placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#6D5EF8]/20 focus:border-[#6D5EF8] transition-all resize-none shadow-sm"
          />
        </div>
        ...
        <div className="pt-2">
          <button
            ...
            className={`w-full text-white font-bold py-3.5 rounded-xl transition-all shadow-md hover:shadow-lg flex items-center justify-center gap-2 ${isProcessing || !text.trim() ? 'bg-[#9CA3AF] cursor-not-allowed' : 'bg-[#EF4444] hover:bg-[#DC2626]'}`}
          >
            {isProcessing ? (
              <><Loader2 className="w-5 h-5 animate-spin" /> Checking...</>
            ) : (
              <><SpellCheck className="w-5 h-5" /> Fix Grammar</>
            )}
          </button>
        </div>
        ...
        <div className="mt-4 pt-4 border-t border-[#E5E7EB]">
          ...
          <div className="flex flex-col gap-2">
            <a href="/tools/ai-writer" className="flex items-center gap-3 p-2.5 bg-[#F9FAFB] hover:bg-[#FEE2E2] rounded-xl transition-colors group">
              ...
              <div>
                <p className="text-[11px] font-bold text-[#111827] group-hover:text-[#EF4444] transition-colors">AI Writer</p>
                <p className="text-[9px] text-[#6B7280]">Write blogs & articles</p>
              </div>
            </a>
            <a href="/tools/ai-summarizer" className="flex items-center gap-3 p-2.5 bg-[#F9FAFB] hover:bg-[#FEE2E2] rounded-xl transition-colors group">
              <div className="w-8 h-8 shrink-0 rounded-lg bg-white border border-[#E5E7EB] flex items-center justify-center group-hover:border-[#EF4444]/30 group-hover:shadow-sm transition-all">
                <LayoutGrid className="w-4 h-4 text-[#EF4444]" />
              </div>
              <div>
                <p className="text-[11px] font-bold text-[#111827] group-hover:text-[#EF4444] transition-colors">AI Summarizer</p>
                <p className="text-[9px] text-[#6B7280]">Summarize long text</p>
              </div>
            </a>
          </div>
        </div>
        ...
        <main className="flex-grow flex flex-col min-w-0">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-2xl font-bold text-[#111827] flex items-center gap-2">
                    AI Grammar Checker <Sparkles className="w-5 h-5 text-[#EF4444]" />
                  </h1>
                  <p className="text-sm text-[#6B7280]">Write flawlessly with instant grammar corrections.</p>
                </div>
              </div>
          ...
          {isProcessing ? (
            <div className="flex-grow bg-white rounded-3xl border border-[#E5E7EB] p-8 flex flex-col items-center justify-center shadow-sm animate-in fade-in duration-500 h-[600px]">
              ...
              <h2 className="text-xl font-bold text-[#111827]">Checking Text...</h2>
              <p className="text-[#6B7280] mt-2">Fixing typos and improving flow.</p>
            </div>
          ) : result ? (
            <div className="flex-grow flex flex-col bg-white border border-[#E5E7EB] rounded-3xl p-6 shadow-sm animate-in zoom-in-95 duration-500 h-[600px]">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-bold text-[#111827]">Corrected Text</h2>
                <div className="flex items-center gap-2">
                  <button 
                    onClick={copyToClipboard}
                    className="flex items-center gap-2 px-4 py-2 bg-[#FEE2E2] text-[#DC2626] rounded-xl text-sm font-bold hover:bg-[#FECACA] transition-colors"
                  >
            ...
            <div className="flex-grow bg-white rounded-3xl border border-[#E5E7EB] p-8 lg:p-12 shadow-sm flex flex-col items-center justify-center text-center relative animate-in fade-in slide-in-from-bottom-8 duration-700 fill-mode-both delay-200 h-[600px]">
              ...
              <div className="relative w-72 h-64 mb-10 flex items-center justify-center mt-4">
                ...
                <div className="relative z-10 w-40 h-40 bg-white rounded-2xl shadow-[0_10px_30px_-10px_rgba(239,68,68,0.3)] border-4 border-red-200 overflow-hidden flex items-center justify-center transform hover:scale-105 transition-transform duration-500">
                  <SpellCheck className="w-16 h-16 text-[#EF4444]" />
                  <div className="absolute bottom-4 right-4 w-6 h-6 bg-green-100 rounded-full flex items-center justify-center animate-bounce">
                    <CheckCircle2 className="w-4 h-4 text-green-600" />
                  </div>
                </div>

                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-48 h-48 border-2 border-dashed border-red-200 rounded-full animate-[spin_10s_linear_infinite]"></div>

                <Sparkles className="absolute top-8 left-12 w-6 h-6 text-red-300 animate-pulse" />
                <Sparkles className="absolute bottom-8 right-12 w-5 h-5 text-red-300 animate-pulse" style={{ animationDelay: '500ms' }} />
              </div>

              <h2 className="text-2xl lg:text-3xl font-bold text-[#111827] mb-3">
                Ready to <span className="text-[#EF4444]">check?</span>
              </h2>
              <p className="text-[#6B7280] max-w-sm mx-auto mb-8">
                Paste your text on the left and click <span className="text-[#EF4444] font-semibold">"Fix Grammar"</span> to improve it.
              </p>
            </div>
          )}
        </main>
