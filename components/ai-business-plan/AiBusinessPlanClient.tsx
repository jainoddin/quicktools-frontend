...
export default function AiBusinessPlanClient() {
  ...
  useEffect(() => {
    if (!isAuthenticated) {
      const count = parseInt(localStorage.getItem('premium_ai-business-plan_count') || '0', 10);
      setFreeGenCount(count);
      const saved = localStorage.getItem('ai-business-plan_history');
      if (saved) {
      ...
      .then(data => {
        if (data.success && data.data?.history) {
          const items = data.data.history
            .filter((item: any) => item.toolSlug === '/tools/ai-business-plan')
            .map((item: any) => ({
              id: item._id,
              prompt: item.prompt,
              result: item.result,
...
Context/Goal: ${formData.context}`;
    ...
    try {
      const res = await fetch(getEndpoint('/api/tools/ai-business-plan'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
    ...
    setToolHistory(prev => {
      ...
      if (!isAuthenticated) {
        localStorage.setItem('ai-business-plan_history', JSON.stringify(updated));
      }
      return updated;
    });
    if (isAuthenticated) {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6 lg:h-full">
      ...
      {showHistory ? (
        ...
        <main className="flex-grow flex flex-col min-w-0">
          ...
          <div className="flex flex-col md:flex-row md:items-start lg:items-center justify-between gap-4 mb-6">
            <div>
              <h1 className="text-3xl lg:text-4xl font-bold tracking-tight text-[#111827] flex items-center gap-2">AI Business Plan Generator <Sparkles className="w-6 h-6" style={{ color: '#F59E0B' }} /></h1>
                ...
                <div className="flex flex-wrap items-center gap-3 pt-6 border-t border-[#E5E7EB] shrink-0">
                  ...
                  <button
                    onClick={async () => {
                      if (navigator.share) {
                        try {
                          await navigator.share({ title: 'AI Business Plan Generator', text: result });
                        } catch (err) {
      ...
      <TextDownloadModal 
        ...
        content={result} 
        filename="AI Business Plan Generator" 
        toolSlug="ai-business-plan" 
