...
import { 
  Magnet, History, Loader2, Copy, CheckCircle2,
  Sparkles, Video } from 'lucide-react';
...
export default function AiHookClient() {
  ...
  useEffect(() => {
    if (!isAuthenticated) {
      const saved = localStorage.getItem('guestHookHistory');
        ...
        .then(data => {
          if (data.success && data.data?.history) {
            const items = data.data.history
              .filter((item: any) => item.toolSlug === '/tools/ai-hook-generator')
              .map((item: any) => ({
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch(getEndpoint('/api/tools/hook-generator'), {
        method: 'POST',
      ...
      if (data.success) {
        ...
        if (!isAuthenticated) {
          ...
          localStorage.setItem('guestHookHistory', JSON.stringify(newHistory));
        } else {
          setToolHistory(prev => [{
            id: data.usageId || Date.now().toString(),
            prompt: topic.substring(0, 100),
        ...
        <div className="pt-2">
          ...
          >
            {isProcessing ? (
              ...
              <><Magnet className="w-5 h-5" /> Generate Hooks</>
            )}
          </button>
        </div>
          ...
          <div className="flex flex-col gap-2">
            <a href="/tools/ai-video-generator" className="flex items-center gap-3 p-2.5 bg-[#F9FAFB] hover:bg-[#F5F3FF] rounded-xl transition-colors group">
              ...
              <div>
                ...
                <p className="text-[9px] text-[#6B7280]">Generate videos from text</p>
              </div>
            </a>
          </div>
        </div>
        ...
        <main className="flex-grow flex flex-col min-w-0">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-2xl font-bold text-[#111827] flex items-center gap-2">
                    AI Hook Generator <Sparkles className="w-5 h-5 text-[#8B5CF6]" />
                  </h1>
                  <p className="text-sm text-[#6B7280]">Generate catchy, curiosity-inducing hooks for TikTok, Reels, or Shorts.</p>
                </div>
              </div>
          ...
          {isProcessing ? (
            <div className="flex-grow bg-white rounded-3xl border border-[#E5E7EB] p-8 flex flex-col items-center justify-center shadow-sm animate-in fade-in duration-500 h-[600px]">
              ...
              <h2 className="text-xl font-bold text-[#111827]">Brainstorming Hooks...</h2>
              <p className="text-[#6B7280] mt-2">Finding the perfect 3-second attention grabbers.</p>
            </div>
          ) : result ? (
            <div className="flex-grow flex flex-col bg-white border border-[#E5E7EB] rounded-3xl p-6 shadow-sm animate-in zoom-in-95 duration-500 h-[600px]">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-bold text-[#111827]">Generated Hooks</h2>
            ...
            <div className="flex-grow bg-white rounded-3xl border border-[#E5E7EB] p-8 lg:p-12 shadow-sm flex flex-col items-center justify-center text-center relative animate-in fade-in slide-in-from-bottom-8 duration-700 fill-mode-both delay-200 h-[600px]">
              ...
              <div className="relative w-72 h-64 mb-10 flex items-center justify-center mt-4">
                ...
                <div className="relative z-10 w-40 h-40 bg-white rounded-2xl shadow-[0_10px_30px_-10px_rgba(139,92,246,0.3)] border-4 border-purple-200 overflow-hidden flex flex-col items-center justify-center transform hover:scale-110 transition-transform duration-500">
                  <Magnet className="w-20 h-20 text-[#8B5CF6] mb-2 drop-shadow-md animate-[bounce_2s_infinite]" />
                </div>
                
                <div className="absolute top-8 left-8 w-8 h-8 bg-white rounded-full shadow-lg border border-[#E5E7EB] flex items-center justify-center animate-[pulse_1s_infinite]">
                  <span className="font-bold text-[#8B5CF6]">👀</span>
                </div>
              </div>

              <h2 className="text-2xl lg:text-3xl font-bold text-[#111827] mb-3">
                Need <span className="text-[#8B5CF6]">attention?</span>
              </h2>
              <p className="text-[#6B7280] max-w-sm mx-auto mb-8">
                Enter your video topic on the left and click <span className="text-[#8B5CF6] font-semibold">"Generate Hooks"</span>.
              </p>
            </div>
          )}
        </main>
